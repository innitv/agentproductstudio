...
Переменные ставятся только в процессе скрипта и не протекают в продуктовую сессию; ключ берётся из `DEEPSEEK_API_KEY` в окружении или `.env`. Смешать оркестратор на Opus с субагентами на DeepSeek внутри одной сессии нельзя — `ANTHROPIC_BASE_URL` один на процесс.
