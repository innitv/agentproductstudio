...
| Противоречие документов о поле контракта | Проза, называющая поле вывода агента опциональным, когда схема требует его (и наоборот). Повод: норма записи `skills_used` была разложена по семи документам и проверена грепом — «правило доведено», — а контракт вывода в том же репозитории называл поле опциональным. Одного слова хватало, чтобы отчёт `workflow:skill-usage` навсегда остался без знаменателя. Греп считает упоминания, но не читает, что сказано; схема валидирует данные, а не прозу о данных. Условная обязательность («обязателен для `product_ui`») конфликтом не считается: схема такого выразить не умеет | `findContractConflicts` в `runtime/typescript/contract-consistency.ts` | `yarn validate:config` (→ `qa:quick`) и `yarn workflow:test-contract-consistency` |
