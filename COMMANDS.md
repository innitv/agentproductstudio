...
### Отдельная сессия на бэкенде DeepSeek

Для механической работы, которая проверяется машинно, можно поднять вторую сессию Claude Code на DeepSeek — дешевле по токенам, 1M контекста:

```powershell
./tooling/scripts/claude-deepseek.ps1        # основная модель flash
./tooling/scripts/claude-deepseek.ps1 -Pro   # основная модель pro
```

Переменные ставятся только в процессе скрипта и не протекают в продуктовую сессию; ключ берётся из `DEEPSEEK_API_KEY` в окружении или `.env`. Смешать оркестратор на Opus с субагентами на DeepSeek внутри одной сессии нельзя — `ANTHROPIC_BASE_URL` один на процесс. Скрипт поднимает отдельный `CLAUDE_CONFIG_DIR`: без этого Claude Code с активной подпиской шлёт в DeepSeek сохранённые креды и получает 401. Проверено 2026-08-06, включая работу инструментов.

🔴 В этой сессии нет vision: визуальные гейты `8.5a`/`8.5b`, `visual-diff`, `reference-check` и Figma write там не работают. Продуктовые стадии со статусом `success`, external write и клиентские материалы туда тоже не уходят. Полная раскладка задач — `docs/architecture/deepseek-delegation.md`.
...
Skills (`.claude/skills/`) slash-команд не имеют: Claude Code подключает их автоматически по описанию. Покрытие стадий: `yarn workflow:skills`; фактическое применение — `yarn workflow:skill-usage`.

Устройство навыка (норма — `agent-pack/templates/skill.template.md`): тело `SKILL.md` инжектируется субагенту целиком при каждом вызове стадии, поэтому в нём остаётся только то, что нужно знать ДО работы и чего нет в контракте — когда навык включается и когда нет, красные правила с причиной, что блокирует готовность. Пошаговые процедуры, полные списки входов и чек-листы уезжают в `references/*.md` рядом с навыком: их читают, когда навык уже запущен. Замер 2026-09-04: перевод девяти навыков на эту схему снял четверть инжекции по всем стадиям, у `01-research` — 46 %.
...
Посмотреть, какие навыки реально открывались на прогонах:

```bash
yarn workflow:skill-usage
```

Отчёт, а не гейт: требовать применения нельзя, навык бывает неприменим к задаче. В знаменатель идут только прогоны, где стадия записала `skills_used` в `handoff-bundle.md` (норма — skill `run-ledger`); без таких записей отчёт прямо говорит, что измерять нечем, и списка «неиспользуемых» не печатает — иначе это был бы ложный ноль.
...
| Противоречие документов о поле контракта | Проза, называющая поле артефакта опциональным, когда схема требует его (и наоборот). Повод: норма записи `skills_used` была разложена по семи документам и проверена грепом — «правило доведено», — а контракт вывода в том же репозитории называл поле опциональным. Одного слова хватало, чтобы отчёт `workflow:skill-usage` навсегда остался без знаменателя. Греп считает упоминания, но не читает, что сказано; схема валидирует данные, а не прозу о данных. Читаются все четырнадцать схем `agent-pack/schemas/`, окно — строка целиком: отрезок символов цеплял соседний заголовок и давал шесть ложных находок. Условная обязательность («обязателен для `product_ui`») конфликтом не считается — схема такого выразить не умеет; поле с разной модальностью в разных схемах из проверки исключается | `findContractConflicts` в `runtime/typescript/contract-consistency.ts` | `yarn validate:config` (→ `qa:quick`) и `yarn workflow:test-contract-consistency` |
| Подпись коммита | `git config user.email`, не совпадающий с `.claude/git-identity`. У владельца два аккаунта GitHub: push шёл под рабочим, а коммиты подписывались почтой второго — GitHub сопоставляет коммит с профилем по e-mail автора, и в истории стоял не тот пользователь. Расхождение заметил человек, хотя оба факта доступны машине. Нет файла ожидания — проверка молчит (fail-open); осознанный обход — env `CLAUDE_ALLOW_IDENTITY_MISMATCH=1` | `.claude/hooks/guard-bash.mjs`, ожидание в `.claude/git-identity` | `yarn workflow:test-guard-bash-identity` (→ `workflow:test-agentic`) |
