...
| Next required artifact | `ia-brief.md` after legal/rails review, custdev plan, or explicit continuation with partial status |
...
- `research-new-rules-retrofit.md`
- `qa-report.md`
- `release-notes.md`
...
- `prd.md` создан как отдельный PRD stage artifact, а не часть research: требования связаны с CJM/research, MoSCoW, user stories, acceptance criteria, analytics и PRD-To-IA/Design handoff. Статус `partial`, потому что legal/rails/custdev gaps остаются открытыми; DeepSeek/Gemini на `01-research` теперь не входят в default-run и могут использоваться только как opt-in advisory checks, не блокирующие readiness сами по себе.
...
- После запроса пользователя о цельном рабочем Notion-артефакте выполнен integrated hybrid pass: database indexes встроены как linked database views в релевантные child pages. Теперь `03 Прото-персоны`, `05 CJM и сценарии`, `06 ICE/RICE бэклог и инициативы`, `10 PRD для MVP` и `08 План валидации и источники` совмещают narrative/context и живую рабочую таблицу.
- Последний локальный research run перепроверен по новым правилам публикации: `research-new-rules-retrofit.md` фиксирует public/internal split, `entity_ownership_map`, `integrated_hybrid` dry-run, lint pass и отсутствие внешней записи.
- После approval `разрешаю публикацию` опубликована новая исправленная research-версия по всем новым правилам: https://app.notion.com/p/37a6473174e581efa178ebd2bdaa422b. Для этой версии создан research-only integrated hybrid layer: 5 баз, 27 rows, 5 linked views встроены в страницы персон, CJM, возможностей, валидации и источников. PRD в этот hub не добавлялся, потому что публикация была research-only.
- По запросу `закинь prd` PRD добавлен в последний hub по новым правилам: https://app.notion.com/p/37a6473174e5817e9542ed85a6dcdcc5. Hub navigation обновлена до 9 child pages; также создана база `A3 Pay Requirements - new rules` на 14 rows и встроена как linked view `Рабочая база требований` внутрь PRD-страницы.
...
|---|---|---|
| Gemini advisory failure | low | Historical legacy auto-run без approval выполнен; DeepSeek прошёл, Gemini вернул `503 Service Unavailable`. По текущим правилам DeepSeek/Gemini не входят в default-run; source-backed readiness не зависит от Gemini. |
...
| Figma-визуализация не заменяет source-backed проверку | low | Использовать canvas как стратегический review artifact; для статуса `ready` нужны source-backed/legal/custdev gates, а DeepSeek/Gemini остаются advisory-сигналом. |
...
| PRD основан на partial research | medium | Не переводить PRD в `ready` до legal/rails review, проверки платежных ограничений и пользовательских тестов. DeepSeek/Gemini failure не является самостоятельным blocker. |
...
| Новая research-only публикация не содержала PRD | closed | PRD добавлен в новый hub отдельным `notion_prd_export`; requirements database на 14 rows встроена в PRD page. |
...
`ia-brief.md` на базе [prd.md](prd.md), если пользователь продолжает pipeline с `partial` статусом; либо legal/rails review и custdev plan, если run нужно переводить ближе к `ready`.

## Legacy Provider Auto-Run Record

- `2026-06-09`: DeepSeek/Gemini advisory check был запущен historical legacy auto-run по прежнему правилу, без approval-вопроса.
- Providers requested: `tavily`, `deepseek`, `gemini`.
- Providers used by штатный `research-stage-runner`: `tavily`, `deepseek`.
- Provider failure: `gemini` вернул `503 Service Unavailable`.
- Итог: этот legacy pass не является текущим требованием; Gemini failure записан как historical advisory failure. По текущим правилам DeepSeek/Gemini не входят в default-run. Readiness остается `partial` из-за legal/rails и custdev gaps.
