 ...
 *
 * История значений: 35 000 → 33 400 (2026-08-17) → **30 000** (2026-08-19).
 *
 * 2026-08-17: аудит показал запас 231 символ, то есть следующая правка индекса уронила бы весь
 * агрегатор. Освободили 1 592 символа: §11.1 сжат до указателя на
 * `docs/architecture/delegation-lessons.md`, описания четырёх кросс-стадийных навыков сжаты до
 * триггера.
 *
 * 2026-08-19: запаса не осталось совсем — правило про новый навык (`context7-docs`) не влезло
 * даже указателем в 62 символа и уехало в `claude-operating-rules.md` §6.2. Разгрузка на 4 007
 * символов, вся — переносом второй копии в её же адрес: правила масштаба из §0.2 в skill
 * `recursive-brief` (шаг 3.3), замеры гейтов человека из §5 в `claude-operating-rules.md` §6.1
 * (индекс сам на них и ссылался), оглавление навыка `shadcn-library` из §6.1 и описания
 * кросс-стадийных навыков в §11 — до повода вызвать, потому что описание каждого навыка и так
 * приходит в контекст его собственным `description`. Новый запас — 436 символов.
 */
export const CLAUDE_MD_CHAR_LIMIT = 30_000;
