...
export const workflowProfiles = ["standard", "reference"] as const satisfies readonly WorkflowProfile[];
...
// Метка «ось не записана», которую ставит скаффолд для оси, значение которой ему не
// передали. Нужна, чтобы гейт опроса остался живым: если бы скаффолд писал раздел
// заглушкой, он закрывал бы гейт сам за себя и проверка стала бы тавтологией. Раздел,
// содержащий метку, считается НЕзаписанным — молчание по-прежнему ошибка, а закрывает
// гейт только явный ответ (переданный флагом на старте либо дописанный оркестратором).
export const intakeSurveyUnrecordedMarker = "[ответ не записан]";
