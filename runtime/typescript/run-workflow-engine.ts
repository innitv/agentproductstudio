...
import { approvalActions, findLatestApprovalRecord, listApprovals, recordApproval, recordApprovalDenial, type ApprovalAction } from "./approval-gate";
...
async function main(): Promise<void> {
  ...
  if (command === "start") {
    const { mode, args } = parseExecutionMode(rest);
    const goal = args.join(" ").trim();
    if (!goal) {
      throw new Error('Usage: yarn workflow:start "<landing workflow goal>" [--mode local|agentic]');
    }

    const state = await startWorkflowEngine({ goal, executionMode: mode });
    console.log(await getWorkflowEngineStatus(state.output_dir));
    return;
  }
...
function parseExecutionMode(args: string[]): { mode: WorkflowExecutionMode; args: string[] } {
  const modeIndex = args.indexOf("--mode");
  if (modeIndex < 0) {
    return { mode: "local", args };
  }

  const mode = args[modeIndex + 1];
  if (mode !== "local" && mode !== "agentic") {
    throw new Error("Execution mode must be one of: local, agentic.");
  }

  return {
    mode,
    args: args.filter((_, index) => index !== modeIndex && index !== modeIndex + 1),
  };
}
...
export function formatAgenticApprovalCommands(
  ...
  if (!rollout.enabledStageIds.length) {
    return [
      ...lines,
      "В текущем rollout нет включённых agentic stages.",
    ].join("\n");
  }
...
export async function formatAgenticPreflight(
  ...
  if (readiness.ready) {
    ...
    if (readiness.blockers.length) {
      sections.push(
        "",
        "- Для strict gate нужно снять blockers или расширить rollout/config так, чтобы заблокированная стадия могла выполниться.",
      );
    }
