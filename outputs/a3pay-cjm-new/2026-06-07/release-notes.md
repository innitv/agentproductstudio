...
`partial`: local research handoff, Notion publication history, Anti-AI-Slop cleanup and PRD stage artifact are complete; full standard product workflow, legal/rails review, custdev checks and downstream IA/design remain open. Gemini failure is recorded as advisory-only.
...
| `stage-gate-ledger.md` | partial | Records publication history and non-blocking historical Gemini advisory failure after legacy auto-run. |
...
Также создан `prd.md` как отдельный stage artifact `02-prd`, а не часть research. PRD содержит Decision Input Audit, Surface Output Contract, Evidence-To-Requirement Map, MoSCoW, user stories, functional/NFR requirements, acceptance criteria, analytics и PRD-To-IA/Design handoff. Статус содержательно `partial`: production-ready scope требует legal/rails review, проверки платежных ограничений и проверки на пользователях. Gemini `503 Service Unavailable` записан как advisory failure и не является самостоятельным blocker.
...
После approval пользователя `разрешаю публикацию` опубликована новая исправленная research-версия по всем новым правилам: https://app.notion.com/p/37a6473174e581efa178ebd2bdaa422b. Hub создан как research-only публикация на 8 child pages и 471 block; затем для него создан integrated hybrid database layer: 5 рабочих баз, 27 rows и 5 linked views внутри страниц персон, CJM, возможностей, валидации и источников. PRD в этот новый hub не публиковался, потому что scope внешней записи был research-only.

После запроса пользователя `закинь prd` PRD добавлен в новый hub по новым правилам: https://app.notion.com/p/37a6473174e5817e9542ed85a6dcdcc5. Hub navigation обновлена до 9 child pages. Для PRD создана база `A3 Pay Requirements - new rules` на 14 rows и linked view `Рабочая база требований` встроена в страницу PRD. После lint-fix в PRD export и Notion page добавлена `CJM-связка PRD` с ключевыми кейсами, вопросами пользователя и сквозным user flow.
...
| `research-summary.md` | 01-research | partial | Source-backed; historical DeepSeek legacy auto-run прошёл, Gemini legacy auto-run вернул `503 Service Unavailable`. |
...
| `research-new-rules-retrofit.md` | 12-release | dry_run_pass | Records latest research run compatibility with new publication rules; no external write. |
...
| Latest research retrofit | `yarn research:lint`, `publish-notion-research-hub.mjs --dry-run`, `research-new-rules-retrofit.md` | pass | Full research pack and public export pass new rules; dry-run has no blockers; external Notion publication not performed. |
| Latest research correction pass | `generate-notion-research-export.mjs`, `yarn research:lint`, `publish-notion-research-hub.mjs --dry-run` | pass | Latest local research/PRD exports cleaned: visible table headers are Russian, provider-debug rows removed from public readiness, validation/source page renamed, `publication_allowed=true`. No external write. |
| New rules Notion publication | `workflow:approve`, `publish-notion-research-hub.mjs`, Notion MCP `fetch` | pass | Published research-only hub `37a64731-74e5-81ef-a178-ebd2bdaa422b`, 8 child pages, 471 blocks; fetch verification passed. |
| New rules integrated hybrid database layer | Notion MCP create_database/create_pages/create_view/fetch | pass_with_limitations | Created 5 databases, 27 rows and 5 embedded linked views; row counts are based on create responses and page fetch verification. |
| New rules PRD publication | `workflow:approve`, `publish-notion-research-page.mjs`, Notion MCP update_page/create_database/create_pages/create_view/fetch | pass | Published PRD page `37a64731-74e5-817e-9542-ed85a6dcdcc5`, updated hub navigation to 9 child pages, created requirements database with 14 rows and embedded linked view. |
| New rules PRD export lint | `yarn research:lint outputs\a3pay-cjm-new\2026-06-07\notion-prd-export.md` | pass | PRD export passes after adding CJM key cases, user questions and сквозной user flow. |
| DeepSeek/Gemini legacy auto-run | `yarn tsx runtime\typescript\research-stage-runner.ts ...` | partial | Historical pass по прежнему правилу: approval не запрашивался; DeepSeek прошёл, Gemini вернул `503 Service Unavailable`; слабый generated candidate pack не принят. Текущие правила больше не включают DeepSeek/Gemini в default-run. |

## Remaining Risks

| Risk | Severity | Owner | Follow-up |
|---|---|---|---|
| Status is `partial`, not `ready` | medium | orchestrator | Нужны legal/rails и custdev checks; повтор Gemini можно сделать позже только как дополнительный advisory check. |
...
| PRD based on partial evidence | medium | product/research | Провести legal/rails review, реальные интервью и prototype tests before `ready`; DeepSeek/Gemini можно включить только явно как дополнительный advisory check. |
...
| New rules hub lacked PRD initially | closed | orchestrator/notion-publisher | PRD is now published in the new hub and requirements database is embedded in the PRD page. |
...
| Remaining blockers | none or advisory provider failure recorded | legal/rails/custdev product-readiness blockers | hold |
...
| `08 План валидации и источники` | Рабочая база гипотез для проверки | embedded |
| `08 План валидации и источники` | Рабочая база источников | embedded |
