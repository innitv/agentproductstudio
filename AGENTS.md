...
- `external write`: Notion, Figma, deploy, изменение секретов, удаление данных и git write без текущего явного запроса требуют exact approval. Model-provider calls требуют approval, кроме обязательных DeepSeek/Gemini cross-check на `01-research`, описанных в разделе Research.
...
DeepSeek/Gemini research auto-run rule: для стадии `01-research` вызовы DeepSeek и Gemini, необходимые только для cross-check, contradiction review и claims-to-validate, считаются обязательной частью research pipeline и **не требуют отдельного human approval/provider opt-in**. Агент должен запускать их автоматически при наличии ключей/провайдеров, фиксировать provider, timestamp, prompt scope, краткий результат и failures в `source-log.md`, `research-summary.md`, `stage-gate-ledger.md` и `handoff-bundle.md`. Запрещено использовать этот auto-run для внешней публикации, записи в Notion/Figma, deploy, git write, отправки сообщений, изменения секретов или любых действий, где меняется внешняя система. Если DeepSeek/Gemini недоступны, stage остается `partial`, а blocker записывается без дополнительного approval-вопроса.
...
- agentic `model_provider_call`, когда stage input покидает локальную песочницу и отправляется в model provider, кроме обязательных DeepSeek/Gemini cross-check на `01-research`, где действует Research auto-run rule.
