...
- **Правило Figma-макетов**: Подключи Figma на этапе дизайна. Если пользователь явно запрашивает отрисовку макетов в Figma (при этом параметр `write_allowed` в Figma MCP должен быть `true` и получено явное одобрение пользователя на внешнюю запись), создай фреймы/макеты непосредственно на холсте Figma перед переходом к разработке в строгом соответствии с [figma-canvas-write-guide.md](file:///c:/Project/product-agent-studio/integrations/mcp/figma-canvas-write-guide.md). В противном случае (по умолчанию, если явного запроса не было) веди дизайн исключительно в виде текстовых локальных спецификаций (`design-brief.md`, `screens.md`).
