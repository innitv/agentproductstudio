...
- Отдельный продукт личного портфолио: `siteportfolio/`. Его активный ledger находится в `siteportfolio/runs/2026-06-14/`, а frontend route — `/portfolio` через `apps/frontend/src/views/PortfolioView.tsx`.
...
Если запрос пользователя относится к `siteportfolio`, не создавай новый `outputs/<project-slug>/...` без явного запроса на полный новый workflow. Для UI-правок работай с `PortfolioView.tsx`, `.portfolio-*` стилями и актуальным ledger в `siteportfolio/runs/`.
