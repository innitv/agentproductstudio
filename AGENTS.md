...
- Для `deep_research` обязательный multi-source default: `tavily` + `deepseek`. Tavily даёт source-backed web/API evidence, DeepSeek используется обязательно для research checks, contradiction review и claims-to-validate, но его synthesis не считается source-backed evidence без внешних источников.
