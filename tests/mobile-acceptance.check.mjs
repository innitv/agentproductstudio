/**
 * ШАБЛОН: мобильная приёмка в профиле устройства (Mobile Device Acceptance Gate).
 *
...
const CONFIG = {
  ...
  app: {
    /** Скроллер основного экрана: селектор или ":root". */
  ...
  engine: {
    /**
     * Что именно Chromium с профилем устройства не воспроизводит. Попадает в
     * строку `engine_limitation`. Дополняй под продукт, но не опустошай:
     * пустой список означал бы «ограничений нет», а это неправда.
     */
    unverified: [
  ...
  scenarios: {
    ...
    // Баг-прародитель: свайп с карточки не листал страницу — pointer-обработчик
    // глушил жест, а `overflow-x` делал ряд скроллером в обеих осях.
    touch_scroll_from_content: {
      route: "/#card-request",
      /*
       * Ряд категорий лежит ниже первого экрана, а жест подаётся в координатах
       * вьюпорта — по элементу за его пределами свайп просто не начнётся.
       * `tap` в prelude авто-прокручивает элемент в вид, поэтому он и играет
       * роль прокрутки: нажатие по чипу — обратимое действие (выбор категории),
       * состояние страницы после него остаётся валидным.
       */
      prelude: [{ tap: '[data-testid="card-request-category-education"]' }],
      /** Свайп ОБЯЗАН начинаться на интерактивном элементе, не на пустом фоне. */
      touchTarget: '[data-testid="card-request-category-taxi"]',
      /**
       * Горизонтальный скроллер (карусель, ряд карточек, таб-бар).
       * Если его в интерфейсе нет — поставь null: горизонтальная ось будет
       * помечена как неприменимая, вертикальная проверится всё равно.
       */
      horizontalScroller: '[data-testid="card-request-categories"]',
      /** Минимальный сдвиг в px, который считается «прокрутилось». */
      minDelta: 40,
    },

    // ── 2. Fixed/sticky и фон против safe-area ───────────────────────────
    // Баг-прародитель: в системных зонах проступали серые полосы — фон страницы
    // «протекал» туда чужим цветом, а theme-color переписывался на переходах.
    safe_area_and_fixed: {
      route: "/#card-request",
      prelude: [],
      /** Закреплённые панели: не должны заезжать за границы вьюпорта. */
      stickyPanels: [
        '[data-testid="card-request-topbar"]',
        '[data-testid="card-request-actionbar"]',
      ],
      /**
       ...
       * системные панели неверно на одном из них. Тонирование остаётся за
       * фоном документа.
       */
      themeColor: null,
      /** Требовать ли `viewport-fit=cover` в meta viewport. */
      requireViewportFitCover: true,
      /** Ожидаемый цвет канвы в системных зонах, [r, g, b] — `--neutral-10`. */
      expectedCanvasColor: [248, 249, 250],
      /** Допуск по каналу: сглаживание и шаг градиента дают ±1. */
      colorTolerance: 1,
    },

    // ── 3. Появление оверлея/баннера/клавиатуры ──────────────────────────
    // Баг-прародитель: пуш-баннер приходил обрезанным и сжимал страницу.
    overlay_appearance: {
      route: "/#card-request",
      /** Шаги до состояния, из которого оверлей вызывается. */
      prelude: [],
      /*
       * Триггер — «Сохранить черновик»: он показывает тост и НЕ меняет высоту
       * страницы. Отправка на согласование сюда не годится: при ошибке она
       * дорисовывает уведомление и подписи ошибок в поток, то есть меняет
       * высоту документа по существу, а не из-за оверлея.
       */
      trigger: '[data-testid="card-request-draft"]',
      /** Сам оверлей: обязан появиться и быть виден целиком. */
      overlay: '[data-testid="card-request-toast"]',
      /**
       * Якорь композиции: элемент, который НЕ должен сдвинуться от появления
       * оверлея (обычно нижняя панель CTA). Так ловится сжатие страницы.
       */
      layoutAnchor: '[data-testid="card-request-actionbar"]',
      /** Сколько ждать, пока пружина выезда успокоится. */
      settleMs: 1500,
    },

    // ── 4. Композиция на реальных ширинах устройств ──────────────────────
    // Баг-прародитель: экран успеха разваливался на реальной высоте Safari,
    // а на «полной» высоте корпуса выглядел правильным.
    composition_widths: {
      route: "/#card-request",
      prelude: [],
      /** Минимум две реальные ширины: одна скрывает переполнение. */
      widths: [390, 430],
      /**
       * Высоты — ВИДИМАЯ область браузера на устройстве, она всегда меньше
       * высоты корпуса на высоту панелей. Именно на ней композиция и ломается.
       */
      heights: [664, 852],
      /** Элементы, которые обязаны присутствовать и быть видны целиком. */
      anchors: [
        '[data-testid="card-request-title"]',
        '[data-testid="card-request-actionbar"]',
        '[data-testid="card-request-submit"]',
        '[data-testid="card-request-draft"]',
      ],
      /**
       * Пары, которые не должны наезжать друг на друга (контент vs кнопка).
       * Отсутствие любого элемента пары — провал: пара описывает композицию,
       * которая обязана существовать на всех перечисленных размерах.
       */
      mustNotOverlap: [
        // Две кнопки в одной строке панели — классическая точка слипания на 390px.
        ['[data-testid="card-request-draft"]', '[data-testid="card-request-submit"]'],
        // Заголовок против панели действий: ловит экран, где панель съела шапку.
        ['[data-testid="card-request-title"]', '[data-testid="card-request-actionbar"]'],
      ],
    },

    // ── 5. Позиция прокрутки при смене состояния ─────────────────────────
    // Баг-прародитель: появление пуша дёргало страницу двумя рывками — узел
    // скролл-контейнера размонтировался (смена ключа AnimatePresence), а
    // `scrollTop` живёт в DOM, не в состоянии React.
    scroll_position_on_state_change: {
      route: "/#card-request",
      prelude: [],
      /** Куда отмотать перед действием: число px или "third" (треть хода). */
      scrollTo: "third",
      /*
       * Триггер — сохранение черновика: состояние меняется (появляется тост),
       * поток страницы не меняется, кнопка живёт в фиксированном доке и тап по
       * ней не прокручивает страницу сам по себе.
       *
       * Отправка с ошибками валидации сюда НЕ подходит и это разобрано, а не
       * обойдено: она дорисовывает уведомление и подписи ошибок ВЫШЕ текущей
       * позиции, Chromium компенсирует вставку scroll anchoring и `scrollTop`
       * законно растёт на высоту вставленного (замерено: 637 → 763 при том же
       * узле контейнера). Визуального прыжка при этом нет, а метрика сценария
       * его не различает — поэтому такой триггер дал бы ложный `fail`.
       */
      trigger: '[data-testid="card-request-draft"]',
      /** Что должно появиться после действия (признак смены состояния). */
      appears: '[data-testid="card-request-toast"]',
      settleMs: 1500,
    },
  },
};
