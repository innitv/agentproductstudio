 ...
 * маршрут #card-request-shadcn — пилотный экран студии. Запуск — `yarn qa:mobile`.
 *
 * ─── МАРШРУТ ВЕРНУЛСЯ НА ПИЛОТНЫЙ ЭКРАН 2026-08-03 ──────────────────────────
 * До этого CONFIG стоял на странице кейса портфолио (run
 * `outputs/portfolio-redesign-seconddesk/2026-08-01`). Портфолио уехало в
 * `C:/Project/siteportfolio` вместе со своей приёмкой, экран А3 удалён
 * решением владельца — в верстаке остался один экран, и приёмка вернулась на
 * него.
 *
 * Смена маршрута ВЕРНУЛА два сценария из `skip`, и это не формальность: на
 * страницах портфолио не было ни оверлеев, ни смены состояния на месте —
 * проверять было нечего. Здесь форма: выпадающее меню даёт всплывающий слой
 * (сценарий 3), выбор «перевыпуск» раскрывает поле и перестраивает DOM
 * (сценарий 5), а фиксированная панель действий несёт
 * `pb-[env(safe-area-inset-bottom)]` — ровно предмет сценария 2.
 *
 * ─── ПОЧЕМУ МАРШРУТ ЗДЕСЬ ОДИН ──────────────────────────────────────────────
 * CONFIG проверяет ОДИН экран за прогон. При добавлении второго продуктового
 * экрана незаменённый CONFIG проверял бы чужую страницу и выдавал ложное
 * «пройдено» — зелёный отчёт про экран, которого никто не трогал. Поэтому при
 * появлении нового продуктового экрана маршрут переключается на него, а
 * прежний остаётся покрыт витриной и скриншот-регрессией; отдельный прогон
 * для него запускается с флагом `--out=`.
 *
...
const CONFIG = {
  ...
  app: {
    /**
     * Страница кейса прокручивает сам документ. Внутренних скроллеров на ней
     * нет ни одного: липкая полоса оглавления с горизонтальной лентой снята
     * 2026-08-02, страница обычная.
     */
  ...
  scenarios: {
    ...
    touch_scroll_from_content: {
      ...
      prelude: [],
      /*
       * Свайп начинается НА содержательном элементе, а не на пустом фоне
       * рядом. Горизонтальных скроллеров на форме нет (ни `overflow-x`, ни
       * scroll-snap), поэтому вторая ось не проверяется: перехватывать жест
       * нечему.
       */
      touchTarget: '[data-testid="card-request-shadcn-title"]',
    ...
    safe_area_and_fixed: {
      ...
      prelude: [],
      /*
       * У этого экрана закреплённых слоёв два, и оба содержательны для
       * системных зон: липкая шапка сверху (`sticky top-0`) и фиксированная
       * панель действий снизу (`fixed inset-x-0 bottom-0`). Нижняя несёт
       * `pb-[env(safe-area-inset-bottom)]` — ровно то, что проверяет этот
       * сценарий: без отступа кнопка отправки уезжает под индикатор жеста.
       */
      stickyPanels: [
      ...
      /*
       * `theme-color` не задаётся: под одним документом живут экраны студии,
       * и единый тег красил бы системные панели по чужой теме.
       */
      themeColor: null,
      requireViewportFitCover: true,
      /**
       * Ожидаемый цвет канвы — `--background` темы `default`: `oklch(1 0 0)`,
       * то есть #ffffff. Красит его правило `html[data-shadcn-theme]` из
       * сгенерированных токенов: атрибут зеркалится на корень документа
       * именно ради этой зоны.
       */
    ...
    overlay_appearance: {
      /*
       * Сценарий вернулся из `skip` 2026-08-03 вместе со сменой маршрута: на
       * страницах портфолио оверлеев не было вовсе, а здесь форма с полями
       * ввода и выпадающим меню — то есть и клавиатура, и всплывающий слой.
       * Проверяется, что появление оверлея не сдвигает разметку под ним.
       */
      route: "/#card-request-shadcn",
      prelude: [],
      trigger: '[data-testid="card-request-shadcn-menu-trigger"]',
      overlay: '[role="menu"]',
      layoutAnchor: '[data-testid="card-request-shadcn-title"]',
      settleMs: 1200,
    },
    ...
    composition_widths: {
      ...
      heights: [664, 852],
      /*
       * Якоря обязаны целиком помещаться в НАЧАЛЬНЫЙ вьюпорт, поэтому здесь
       * только первый экран: липкая шапка и заголовок формы. Панель действий
       * закреплена внизу и в начальный вьюпорт попадает всегда — её положение
       * проверяет сценарий 2, а не этот.
       */
      anchors: [
        '[data-testid="card-request-shadcn-topbar"]',
      ...
      /*
       * Заголовок не должен попадать под липкую шапку: на 390 в шапке
       * переносятся крошки, и наезд возник бы раньше всего именно здесь.
       * Вторая пара — заголовок и фиксированная панель снизу: на высоте 664
       * короткий вьюпорт сближает их сильнее всего.
       */
      mustNotOverlap: [
        [
          '[data-testid="card-request-shadcn-topbar"]',
    ...
    scroll_position_on_state_change: {
      /*
       * Тоже вернулся из `skip` 2026-08-03: на портфолио состояние на месте не
       * менялось, а форма перестраивает DOM — выбор «перевыпуск» раскрывает
       * поле номера карты. Проверяется, что прокрутка при этом остаётся на
       * месте: раскрытие блока выше точки просмотра сдвигает содержимое, и
       * браузер способен увести пользователя в другое место страницы.
       */
      route: "/#card-request-shadcn",
      prelude: [],
      scrollTo: 600,
      trigger: '[data-testid="card-request-shadcn-purpose-reissue"]',
      appears: '[data-testid="card-request-shadcn-reissue-number"]',
      settleMs: 800,
    },
  },
};
