 ...
 * маршрут #a3-finance — информационную страницу ООО РНКО «А3 Финанс».
 * Запуск — `yarn qa:mobile`.
 *
 * ─── ПОЧЕМУ МАРШРУТ ЗДЕСЬ ОДИН ──────────────────────────────────────────────
 * CONFIG проверяет ОДИН экран за прогон. Раньше здесь стоял пилотный
 * `#card-request-shadcn`; при добавлении второго продуктового экрана
 * незаменённый CONFIG проверял бы чужую страницу и выдавал ложное «пройдено» —
 * зелёный отчёт про экран, которого никто не трогал. Поэтому маршрут
 * переключён на страницу текущего run. Пилотный экран остаётся покрыт
 * витриной и скриншот-регрессией; мобильная приёмка для него, если
 * понадобится, запускается отдельным прогоном с флагом `--out=`.
 *
...
const CONFIG = {
  ...
  app: {
    /** Страница А3 прокручивает сам документ: внутренних скроллеров нет. */
  ...
  scenarios: {
    ...
    touch_scroll_from_content: {
      route: "/#a3-finance",
      /*
       * Prelude пуст намеренно: цель жеста видна сразу после загрузки, а
       * лишний тап по кнопке hero увёл бы страницу к форме и сделал бы замер
       * прокрутки нечитаемым.
       */
      prelude: [],
      /** Свайп ОБЯЗАН начинаться на интерактивном элементе, не на пустом фоне. */
      touchTarget: '[data-testid="a3-hero-cta"]',
      /*
       * Горизонтальных скроллеров на странице нет вовсе: восемь блоков идут
       * вертикальным потоком, каруселей и таблиц с прокруткой в образце нет.
    ...
    safe_area_and_fixed: {
      route: "/#a3-finance",
      prelude: [],
      /** Два закреплённых слоя страницы: шапка сверху и бар согласия снизу. */
      stickyPanels: ['[data-testid="a3-header"]', '[data-testid="a3-cookie-bar"]'],
      /**
       * `theme-color` не используется: верх страницы белый, низ — ink-navy
       * футера, и единый тег красил бы системные панели неверно на одном из
       * концов. Под одним документом живут ещё и экраны других тем.
       */
      themeColor: null,
      requireViewportFitCover: true,
      /**
       * Ожидаемый цвет канвы — `--background` темы `a3`, то есть `#ffffff`.
       ...
       *
       * Значение привязано к теме: смена фона темы потребует замерить пиксель
       * заново, иначе сценарий поймает не «системная зона осталась
       * некрашеной», а собственную устаревшую цифру.
       */
    ...
    overlay_appearance: {
      route: "/#a3-finance",
      /*
       * Первый тап прокручивает строку реквизита в вид и показывает тост.
       * Он нужен именно как prelude: тогда ВТОРОЙ тап (сам триггер) уже не
       * двигает страницу, и сдвиг якоря композиции означает настоящий layout
       * shift, а не автопрокрутку Playwright к цели.
       */
      prelude: [{ tap: '[data-testid="a3-requisite-inn-copy"]' }],
      trigger: '[data-testid="a3-requisite-inn-copy"]',
      /** Уведомление sonner: собственная разметка библиотеки, портал на body. */
      overlay: "[data-sonner-toast]",
      /** Якорь композиции: футер не должен сдвинуться от появления тоста. */
      layoutAnchor: '[data-testid="a3-footer"]',
      settleMs: 1500,
    },
    ...
    composition_widths: {
      ...
      heights: [664, 852],
      /*
       * Якоря обязаны быть видны в НАЧАЛЬНОМ вьюпорте: проверка требует, чтобы
       * бокс целиком лежал в его границах. Поэтому здесь первый экран и два
       * закреплённых слоя, а не элементы из середины страницы.
       */
      anchors: [
        '[data-testid="a3-hero-title"]',
       ...
       * Сначала стояла пара «бар согласия × кнопка hero». Она упала на
       * 390×664 и 430×664, и замер объяснил почему: бар 192 px высотой, кнопка
       * занимает 455…503, до низа вьюпорта от неё остаётся 161 px. Текст бара —
       * 116 знаков, три строки при 14/22, плюс тач-таргет 44: в 161 px он не
       * помещается ни при какой вёрстке. То есть падала не композиция, а сама
       * проверка: она требовала невозможного от нижнего фиксированного бара,
       * который по устройству закрывает низ первого экрана, а кнопка
       * становится доступной после любой прокрутки.
       *
       * Заменено на пары, которые проверяют то, что действительно обязано
       * выполняться: бар не съедает опознание страницы, а sticky-шапка не
       * наезжает на H1. Обе на 664 проходят с запасом и упадут, если бар или
       * шапка вырастут.
       */
      mustNotOverlap: [
        ['[data-testid="a3-cookie-bar"]', '[data-testid="a3-hero-title"]'],
        ['[data-testid="a3-header"]', '[data-testid="a3-hero-title"]'],
      ],
    },
    ...
    scroll_position_on_state_change: {
      ...
      scrollTo: "third",
      /*
       * Бургер живёт в sticky-шапке и виден при любой позиции прокрутки —
       * значит тап по нему не двигает страницу сам по себе, и замер меряет
       * именно смену состояния. Открытие панели меню блокирует прокрутку тела
       * документа: если бы блокировка сбрасывала позицию, пользователь после
       * закрытия меню оказывался бы в начале страницы.
       */
      trigger: '[data-testid="a3-menu-toggle"]',
      appears: '[data-testid="a3-mobile-menu"]',
      settleMs: 800,
    },
  },
};
