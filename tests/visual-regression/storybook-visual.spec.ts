...
const excludedStories: Record<string, string> = {
  // Пусто, и это полное покрытие, а не забытый список. Обе прежние записи —
  // полностраничные истории экранов А3 и портфолио — удалены 2026-08-03 вместе
  // с самими экранами: портфолио уехало в `C:/Project/siteportfolio`, А3 удалён
  // решением владельца. Исключения существовали ровно потому, что те страницы
  // были выше 4500 px и в кадр 1280×2000 не помещались; у оставшегося пилотного
  // экрана такой проблемы нет.
};
