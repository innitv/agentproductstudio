/**
 * Мобильная приёмка shadcn-версии экрана заявки (Mobile Device Acceptance Gate).
 *
 * Копия каркаса tests/mobile-acceptance.check.mjs с CONFIG под маршрут
 * #card-request-shadcn-branded. Отдельный файл, а не переключатель в исходном:
 * у двух реализаций одного экрана разные селекторы и разный набор применимых
 * сценариев, и смешивать их в одном CONFIG означало бы, что приёмка A3
 * перестала бы быть приёмкой A3.
 *
 ...
 *
 * ─── ЗАПУСК ─────────────────────────────────────────────────────────────────
 *   node tests/mobile-acceptance-shadcn.check.mjs --base=http://127.0.0.1:4173
 *   node tests/mobile-acceptance-shadcn.check.mjs --base=https://<деплой> --out=test-results/mobile
 *
...
const CONFIG = {
  ...
  outDir: "test-results/mobile-acceptance-shadcn",
  ...
  scenarios: {
    ...
    touch_scroll_from_content: {
       ...
       * Горизонтальная ось будет помечена как неприменимая — это отличие
       * shadcn-версии от A3, где ряд чипов прокручивается вбок.
       */
