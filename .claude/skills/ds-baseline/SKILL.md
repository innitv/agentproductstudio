...
Skill применяется, когда `design_system_mode = product_specific` или подтверждённый `extend` требует нового foundation. Нормативный процесс — [`agent-pack/workflows/ds-baseline.workflow.md`](../../../agent-pack/workflows/ds-baseline.workflow.md); техника записи в Figma — skill `/figma-ds:build`, канон DS — `/figma-ds:standard` (плагин `figma-ds`, `plugins/figma-ds/`, раздаётся всем проектам); выбор самого режима — [`figma-roundtrip`](../figma-roundtrip/SKILL.md).
