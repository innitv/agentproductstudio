...
7. **`skills_used`:** перечисли навыки, которые этап реально открывал, строкой `skills_used: id, id`. Пустая строка — законный ответ: навык мог быть неприменим. Это не гейт, а материал для `yarn workflow:skill-usage`: без записи отчёт не может отличить «навык не нужен» от «применение никто не фиксирует», и вопрос «держим ли мы навык, который никто не открывал» остаётся без ответа. Цена умолчания измерима — тела навыков из `skills:` инжектируются субагенту целиком при каждом вызове стадии.
...
Definition of Done для этапа: обязательные артефакты созданы/обновлены; `inputs_used` и `skills_used` зафиксированы; `handoff-bundle.md` и `stage-gate-ledger.md` обновлены; validation выполнена или blocker записан.
...
🔴 **Ставь строку командой:** `yarn workflow:human-review <run-dir> <7.5|8.5a|8.5b> --notes "…" [--shown "…"]`. Руками её не писали ни в одном из десяти прогонов (аудит 2026-08-17) — отсюда команда.

Валидатор требует все три точки: `8.5a`/`8.5b` — после отработавшей `08-frontend`, `7.5` — если в каталоге run лежит `figma-layout-ir.json` (признак того, что экраны собирались в Figma). До 2026-08-21 точка `7.5` не проверялась ничем, хотя была объявлена нерушимой.
...
🔴 **Маркер без датированного `##`-заголовка невидим.** `yarn workflow:retro` ищет **заголовок**, а маркер читает как его атрибут: маркер, поставленный строкой списка, ячейкой таблицы или внутри абзаца, не привязан ни к какому заходу и в разбор не попадает. Прецедент `a3pay-subscriptions-widget` (2026-08-03): оркестратор аккуратно проставил `<!-- retro: pass=N found_by=user_review -->` **восемь раз** — но строками внутри раздела «Машиночитаемые записи гейтов», а не заголовками заходов. Ретро напечатало канал `unknown` для 9 заходов из 9, то есть ровно тот же результат, что при полном отсутствии разметки. Работа была сделана и пропала.

**С 2026-08-04 это проверяется машинно.** `yarn workflow:validate` даёт **error** на каждый артефакт, где есть датированные заходы без маркера под заголовком, и называет номера строк. Норма держалась на дисциплине три прогона подряд и три раза не сработала — теперь её держит проверка. Быстрый контроль на месте прежний: `yarn workflow:retro <run-dir>`, канал `unknown` означает, что разметка не привязалась.

🔴 **Четвёртый прецедент, и он про предел самой нормы.** `a3-brand-presentation-template` (2026-08-05…06): семь раундов проб в Figma, порядка двадцати пяти проб отклонено человеком с прямыми формулировками («мимо», «слишком дёшево и пусто», «вычурный»). Все заходы записаны разделами ledger без маркеров, `yarn workflow:validate` не запускался **ни разу за сессию** — первый прогон случился на ретро и немедленно дал `ERROR` с номерами строк 83, 134, 260, 303, 350, 356. Метрика снова напечатала **0 дорогих находок**.

Вывод, который стоит держать в голове при чтении этого раздела: **машинная проверка, требующая ручного вызова, — это та же дисциплина, только с лишним шагом.** Четыре прогона подряд норма не удержалась, и её пятая формулировка ничего не изменит. Нужен механизм, вызывающий проверку без участия оркестратора (хук на запись `human_review`, привязка к отправке файлов человеку) — это предмет `/subsystem-audit:audit`, не ретро. Разбор: `docs/architecture/retro-a3-brand-presentation-template-2026-08-06.md`.

### 4.2. Артефакт, переписываемый заходами, проверяется валидатором после каждого захода

🔴 **Прогнать `yarn workflow:validate <run-dir>` по своей стадии — часть захода, а не финала run.** Артефакт, который переписывают много раз подряд, теряет структуру постепенно и молча: каждый отдельный заход выглядит правильным, а схема ломается на третьем-четвёртом.

Прецедент `a3pay-subscriptions-widget` (2026-08-03…04): `screens.md` пережил девять заходов. Ранние агенты валидатор прогоняли и отчитывались «0 errors по стадии», поздние — перестали. На закрытии run валидатор дал **55 ошибок, из них 46 одного класса**: schema validation артефакта и десять отсутствующих обязательных секций. Ни один заход в отдельности этого не создавал — секции вымывались правками по очереди.

Стоимость проверки — одна команда; стоимость пропуска — артефакт, который нельзя сдать и который никто не перечитает целиком. Если валидатор ругается на чужую стадию, это не твоя забота: смотри строки со своим `<stage-id>`.
...
- [ ] `skills_used` записан (пустой список — тоже запись).
