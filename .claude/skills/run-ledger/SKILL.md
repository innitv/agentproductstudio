...
Проверка на месте, одной командой: `yarn workflow:retro <run-dir>` после захода. Если в колонке «Канал» стоит `unknown` — разметка не сработала, чинить сейчас, а не на ретро через неделю.
