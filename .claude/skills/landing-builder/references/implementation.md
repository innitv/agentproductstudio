# Реализация фронтенда: входы, процедура, архитектура, приёмка

Детали для `landing-builder`. Читать, когда skill запущен — то есть когда
артефакты на месте и начинается работа с кодом.
