...
4a. 🔴 **Если кадр ВОСПРОИЗВОДИТ существующий объект** (миниатюра схемы в инвентаре, знак из каталога, слайд-носитель внутри листа) — сверка со страницей целиком не годится: на общем скриншоте копия «похожа», а рядом с оригиналом расходится составом. Рисуй копию **из фактической геометрии оригинала** (координаты, размеры, толщины и тона читаются программно, один коэффициент масштаба на объект), снимай **парный кадр оригинал/копия на каждый объект** и сверяй **поэлементным счётом** (узлы, линии, рамки). Имя группы содержимого не гарантирует: группа бывает пустой или носит имя клона-донора — читай геометрию зоны, а не ярлык. Полная норма и прецедент (восемь миниатюр из девяти разошлись при парной съёмке, run `a3-brand-presentation-template` 2026-08-11) — skill `visual-diff-verifier` §3.4.
