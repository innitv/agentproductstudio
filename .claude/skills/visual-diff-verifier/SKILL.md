...
Инструмент вердикта не выносит: часть расхождений — намеренные решения продукта. Классификацию `defect`/`intentional`/`unknown` делает агент `reference-auditor` (или ты сам), а `pass`/`fail` по reference fidelity остаётся за `qa-review`.
