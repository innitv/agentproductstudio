...
**Skill применяется только тогда, когда работа идёт в Figma** — пользователь просит макет в Figma или показ результата человеку. По `CLAUDE.md` §6.1 Figma сузилась до дивергентной фазы на `04-design` и разового показа; Figma-кит компонентов не ведётся, синхронизации с кодом нет. Маршрут читается из `run-state.json` (CLAUDE.md §0.3), а не выводится по наличию файлов.
