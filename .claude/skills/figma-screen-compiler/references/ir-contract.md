# Контракт IR: входы, процедура, критерии блокировки

Детали для `figma-screen-compiler`. Читать, когда skill запущен — то есть перед
Figma canvas write для `figma_board`, `product_ui` или `prototype`.
