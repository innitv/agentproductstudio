...
Нормативные источники: [`agent-pack/guardrails/approval-matrix.md`](../../../agent-pack/guardrails/approval-matrix.md) (что требует approval) и [`agent-pack/workflows/claude-operating-rules.md`](../../../agent-pack/workflows/claude-operating-rules.md), раздел 6 (Interactive Question Gate, Process Deviation Record).
...
1. **Классифицируй действие** по [approval-matrix](../../../agent-pack/guardrails/approval-matrix.md). Локальная запись артефакта и локальная команда валидации approval не требуют. Всё, что уходит наружу, — требует.
