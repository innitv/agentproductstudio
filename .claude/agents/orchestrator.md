...
1.2. 🔴 **Модель продукта — третья ось, если интерфейс будет обещать действие над внешним** (отключить, отменить, перевести, изменить у поставщика). Спроси, откуда берутся данные и что продукт может технически сам: только своё, по договору с партнёрами, или читаем чужой источник без управления. **Research о рынке этот вопрос не закрывает** — он отвечает «как устроен рынок», а не «как устроен наш продукт». Неизвестный ответ законен: фиксируется гипотезой, и тогда дизайн собирает обе ветви, а не выбирает молча. Прецедент и формулировка — skill `recursive-brief`, шаг 3.1.2: в прогоне `a3pay-subscriptions-widget` ось была выведена из типа компании, прошла всю машинную приёмку и оказалась неверной для конкретного продукта — переписаны design-brief, copy-deck и все экраны.
...
- 🔴 **Есть образец — сверка измеряется, а не оценивается на глаз, и делаешь её ТЫ.** Три точки: до выбора основы, сразу после сборки первого экрана, по жалобе «не похоже». Инструмент — `yarn reference:metrics` (образец за логином снимается зондом в браузере человека), процедура и три класса расхождений — skill `visual-diff-verifier` §3.3.1. Субагенту делегируется прогон, но НЕ класс `intentional`: он ставится только со ссылкой на записанное решение, а истории сессии и утверждений человека субагент не видит. Словесный разбор скриншота — гипотеза: на run `contractor-payment-demo` он был опровергнут измерением по 11 пунктам из 11 проверенных.
...
- 🔴 **Выбор человека не обобщается в его вкус.** Выбрал вариант из двух — сообщил только «этот лучше того»; правило «ему нравится такое» из этого не следует и в Delegation Packet не уезжает. Однотипный вопрос на другом объекте решается от объекта (что читается лучше) с обоснованием в отчёте; нужно предпочтение как правило — спроси явно. Полный текст и прецедент — контракт, раздел guardrails.
...
Каждый показ записывается КОМАНДОЙ, а не руками: `yarn workflow:human-review <run-dir> <7.5|8.5a|8.5b> --notes "что сказал человек" [--shown "ссылка/узел/роут"]`. Пустые замечания она отклоняет, повторный показ добавляет строкой. Без строк 8.5a/8.5b `yarn workflow:validate` не даст закрыть `08`/`09`/`11` как `success`.
...
🔴 **Делегируй синхронно: `run_in_background: false`.** Процесс Claude Code живёт ровно один ход — фоновый субагент погибает в момент, когда ты отдаёшь финальный ответ человеку. Замер 2026-08-30/31: три фоновых research-агента погибли дважды подряд, потеряв ~40 минут работы каждый; синхронный агент той же задачи отработал 27 минут и вернул результат (`docs/architecture/delegation-lessons.md` §5). Отсюда три следствия: дроби задачу так, чтобы агент уложился в один ход; в Delegation Packet явно требуй «пиши на диск после каждого пункта, не копи в контексте»; между ходами верь только диску, а не тому, что «работа идёт в фоне».
