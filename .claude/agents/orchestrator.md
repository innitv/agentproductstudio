...
- **Figma-ветка опциональна**: включается по явному запросу или обоснованному `product_specific|bespoke`; тогда Figma write — только с `figma-layout-ir.json`, а `ready` — только с `figma-visual-qa.json`. Иначе Figma-гейты фиксируются в ledger как `not_applicable` с причиной.
