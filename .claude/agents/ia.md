...
- **Куда писать:** `ia-brief.md` → в текущий run-каталог `outputs/<project-slug>/<YYYY-MM-DD>/` (путь даёт оркестратор).

## Что теряется чаще всего — навык `ia-structure`

Тело навыка приходит тебе целиком, отдельно открывать не нужно. Три вещи оттуда, которые решают качество стадии: **валидатор требует 4 секции из 21**, поэтому `ia-brief.md` без State Map и Decision & Friction Map проходит проверку зелёным, а потерянные состояния всплывают на `08-frontend`; **сценарии не переписываются** — источник `scenario-user-flows.md`, IA накладывает на них структуру, а неполный сценарий возвращается в research как gap; **до главного действия ≤ 3 шага** от любой точки входа, иначе записанная причина или пересборка структуры.
