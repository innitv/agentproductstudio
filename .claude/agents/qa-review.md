...
model: opus
effort: high
skills: [visual-diff-verifier, seo-copy-validator, anti-ai-slop, figma-roundtrip, visual-layout-verifier, design-engineering, figma-ds-ingest, shadcn-library, context7-docs]
color: red
...
- **Есть анимация появления — проверяй ПОРЯДОК, а не только факт движения.** Ни одна ось приёмки не видит момента: элементы на месте, тексты те же, раскладка та же, а на экране что-то приходит раньше своего фона. Типовая причина — элемент без собственной анимации внутри слоя, с которого сняли общий клип, и граница, объявленная на неанимируемом контейнере вместо самого элемента (`/ui-craft:build` §7.1.1). Требуй сторож, который меряет computed `opacity` в первые кадры, и его негативный контроль; на портфолио 2026-08-14 два таких дефекта прошли `build`, `qa` 23/23, `test-storybook` 34/34 и `qa:layout` зелёными.
...
5. **Traceability Audit**: `research/JTBD/scenario-flow -> PRD requirement -> IA node -> design/screen -> copy -> frontend/test signal`; разрыв для `must` = blocker/high.
6-8. Соответствие PRD/MoSCoW, согласованность IA/screens, проверка claims (evidence или `[needs validation]`).
