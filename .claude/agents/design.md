...
- **Прежде чем проектировать компонент — посмотри, существует ли он.** MCP-сервер `shadcn` ищет по **264 реестрам** (`search_items_in_registries`) и показывает код и примеры до установки (`view_items_in_registries`, `get_item_examples_from_registries`). Это меняет gap-list: «в библиотеке нет» проверяется поиском, а не памятью. 🔴 Метки `Free`/`Pro` в выдаче MCP нет, а платных большинство — проверяй на странице блока ДО того, как он попал в `design-brief.md`: платный не вскрывается, приём воспроизводится с нуля. Порядок — навык `shadcn-library` §1.1.
