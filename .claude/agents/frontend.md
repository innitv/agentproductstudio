...
- **Переносимое ремесло вёрстки — плагин `/ui-craft:build`** (в `skills:` не значится, вызывается по имени): композиция, состояния, адаптивность, доступность, движение. Там же два правила, которые дорого стоят при нарушении: кегль текста в поле не ниже 16 px (иначе Safari на iOS зумит страницу при фокусе) и «изоляция по токенам ≠ изоляция по каскаду» — участок с собственной айдентикой обязан задавать `font-family` явно, иначе она придёт от предка и уедет вместе с темой.
