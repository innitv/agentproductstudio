...
model: opus
color: purple
skills: [design-loop, figma-screen-compiler, ds-baseline, figma-ds-ingest, approval-gate, visual-layout-verifier, figma-roundtrip, figma-handoff, shadcn-library, presentation-craft]
...
- **`product_layer` ставится после поиска, а не по памяти.** MCP-сервер `shadcn` ищет по **264 реестрам**: `search_items_in_registries` находит, `view_items_in_registries` показывает код до установки. Компонент, объявленный gap-ом без поиска, — необоснованный `product_layer`. 🔴 Метки `Free`/`Pro` в выдаче MCP нет, платных большинство: блок из стороннего реестра проверяется на его странице до попадания в матрицу, платный воспроизводится с нуля и помечается как `product_layer`, а не `shadcn_registry`. Порядок — навык `shadcn-library` §1.1.
...
- 🔴 **Правишь `screens.md` повторным заходом — оформи заход и прогони валидатор своей стадии.** Заход = датированный `##`-заголовок плюс строка `<!-- retro: pass=N found_by=... -->` под ним (маркер вне заголовка машина не видит), затем `yarn workflow:validate <run-dir>` и чтение строк со своим `06-screens`. Прецедент `a3pay-subscriptions-widget`: девять заходов, ранние агенты валидатор гоняли, поздние перестали — на закрытии 46 ошибок одного класса, секции артефакта вымылись правками по очереди, и ни один отдельный заход этого не создавал. Полный текст обеих норм — skill `run-ledger` §4.1 и §4.2.
