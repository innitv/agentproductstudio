...
- Делегируй этап через `Agent` tool с `subagent_type: design`. Цель: собрать `reference-analysis.md` (если есть референс), `design-brief.md`, зафиксировать Design System Strategy Gate (`reuse|extend|product_specific|bespoke`), reuse/gap-компоненты и app-like критерии.
