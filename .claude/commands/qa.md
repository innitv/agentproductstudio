...
Порядок действий:
- Прочитай все ключевые артефакты run (`prd.md`, `ia-brief.md`, `design-brief.md`, `screens.md`, `frontend-result.md`, `visual-reference-review.md` при наличии); зафиксируй `inputs_used`.
- Делегируй этап через `Agent` tool с `subagent_type: qa-review`. Цель: собрать `qa-report.md` с проверкой PRD fit, UX, visual/reference gates, accessibility, responsive и отсутствия секретов.
