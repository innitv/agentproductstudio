...
- Делегируй проверку через `Agent` tool с `subagent_type: qa-review` с задействованием skill `visual-diff-verifier`. Цель: собрать `visual-reference-review.md` с Source Pair Matrix и поблочным сравнением.
...
- **Кадрового diff недостаточно, и сам по себе он отвечает не на тот вопрос.** Он сравнивает КАДРЫ и говорит «похоже ли»; когда реализация переносит образец на свой контент, кадры расходятся полностью и процент различий не значит ничего. Сними ещё и признаки: `yarn reference:metrics --reference=<url|dump.json> --local=<url>` — типографика (фактическая гарнитура отдельно по кириллице), плотность, палитра, профиль высот накопленной координатой. Классифицируй каждое расхождение как `defect` / `intentional` / `unknown` по процедуре skill `visual-diff-verifier` §3.3.1. Класс `intentional` ставишь ТЫ и только со ссылкой на записанное решение — субагенту он недоступен, у него нет истории сессии.
