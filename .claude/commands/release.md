...
- Делегируй этап через `Agent` tool с `subagent_type: release`. Цель: собрать `release-notes.md` с changed files, validation результатами, deployment notes и rollback notes.
- Проверь Completion Gate: если публикация в Notion была запрошена, нужен publication record или явный blocker/partial (см. `/notion-publish`). Публикация не является обязательным шагом workflow — её отсутствие без запроса пропуском не считается.
