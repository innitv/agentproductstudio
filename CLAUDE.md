...
Этот файл — корневая инструкция проекта и канонический операционный контракт для Claude Code. Он должен быть читаемым: здесь находятся правила, которые Claude обязан помнить сразу. Подробные процедуры вынесены в связанные документы и шаблоны. Claude Code загружает этот файл автоматически в начале каждой сессии.
...
- **Специалисты** — субагенты в `.claude/agents/*.md`. Оркестратор вызывает их через `Task` tool (параметр `subagent_type` = имя агента). Каждый субагент работает в изолированном контексте, возвращает структурированный результат и не подменяет общий статус workflow. Полные контракты специалистов (frontmatter, required inputs/outputs, gates) остаются в `agent-pack/agents/*.agent.md` и являются детальным источником правды; `.claude/agents/*.md` — это нативная Claude-обёртка, которая ссылается на них.
...
Ключевой принцип не изменился: **manager-style orchestration**. Главная сессия (оркестратор) держит финальный результат, а специалисты вызываются как ограниченные capabilities. Прямой ответ от специализированного субагента без синтеза оркестратора запрещён для продуктового pipeline.
...
## 0.2. Дисциплина тщательности

Тщательность, проверка источников истины и соблюдение gates важнее скорости видимого результата. Агент не имеет права трактовать запрос как просьбу "сделать быстро", если пользователь явно не сказал `quick draft`, «быстрый набросок», `demo only` или аналогичный режим низкой строгости.

Перед генерацией артефакта, записью в Figma/Notion, frontend implementation, публикацией, handoff downstream-агенту или финальным статусом каждый агент обязан выполнить context/source inventory: какие инструкции, артефакты, references, design-system assets, компоненты, templates, runtime contracts и пользовательские ограничения реально являются источником решения. Новое создается только для доказанного gap; если подходящий источник уже есть, его нужно переиспользовать или расширить минимально.

Для Figma, product UI и дизайн-систем это означает: сначала прочитать/проинспектировать существующую design system, variables/styles, библиотеки, components, variants, component contracts и существующие экраны; затем зафиксировать `reuse|extend|product_specific|bespoke`, список переиспользуемых элементов и список только недостающих gap-компонентов. Собирать новые макеты "с нуля" поверх существующей системы запрещено без explicit deviation.

Если агент нарушил уже существующее правило, это фиксируется как `process_deviation`. Запрещено называть исправление такого нарушения "поправкой пользователя": это не новая вводная пользователя, а восстановление обязательного правила.
...
Claude работает как инженерно-продуктовый агент для сборки веб-интерфейсов, B2B/B2C-консолей, лендингов, прототипов и продуктовых workflow через оркестр субагентов.
...
- `orchestrator` (главная сессия Claude Code) владеет финальным результатом.
- Специалисты вызываются как bounded capabilities / agents-as-tools через `Task` tool с `subagent_type`.
...
- Матрица `кто что получает и что отдает` зафиксирована в `agent-pack/workflows/stage-handoff-contract.md`; при изменении маршрута синхронизируй ее вместе с `runtime/typescript/workflow.manifest.ts`, `.agent.md`, `.claude/agents/*.md`, шаблонами и тестами.
...
Субагенты описаны в `.claude/agents/*.md` (нативные Claude-обёртки) и детально в `agent-pack/agents/*.agent.md`. Route/dependency graph описан в `runtime/typescript/route.config.ts` и `runtime/typescript/workflow-stages.ts`.

Архитектурные границы repo и правила веток описаны в `docs/architecture/repo-map.md` и `docs/architecture/git-workflow.md`. Если меняется app boundary, deploy route или branch policy, обновляй эти документы вместе с `CLAUDE.md`, README и релевантными тестами.

## 3. Рабочий режим

Единственный пользовательский режим проекта — **работа через Claude Code** (CLI в терминале, desktop-приложение, web на claude.ai/code или IDE-расширение).

Пользователь пишет запросы в Claude Code, а Claude читает `CLAUDE.md`, инструкции специалистов, workflow-документы и шаблоны, затем выполняет работу через доступные локальные инструменты и субагентов.
...
Для ограниченных инженерных задач вне полного продуктового workflow можно создать task-scoped ExecPlan по `agent-pack/templates/task-exec-plan.template.md`. Для повторяемых процедур создавай отдельный SOP по `agent-pack/templates/agent-sop.template.md` и подключай его ссылкой из релевантного workflow/agent/skill вместо раздувания `CLAUDE.md`.
...
MCP-серверы проекта объявлены в `.mcp.json` (пример — `integrations/mcp/mcp-servers.example.json`). Для вопросов по Claude Code, MCP или связанной документации используй проектные MCP-инструкции и проверяемую документацию без сохранения внешних ссылок в нормативных файлах.
...
Нативные Claude-обёртки (вызываются через `Task` tool, `subagent_type` = имя):
...
- `.claude/agents/copywriting.md`
- `.claude/agents/frontend.md`
- `.claude/agents/test-bench.md`
- `.claude/agents/qa-review.md`
- `.claude/agents/release.md`

Детальные контракты специалистов (frontmatter, required inputs/outputs, gates, output contract) — в `agent-pack/agents/*.agent.md`. Для повторяющихся технических действий используй skills из `.claude/skills/` (детально — `agent-pack/skills/`), например `landing-builder` и `visual-diff-verifier`.

## 13. Триггер-фразы и slash-команды

Триггер-фразы работают в свободном чате; им соответствуют slash-команды в `.claude/commands/`.

Глобальные:

- Старт: `начать воркфлоу`, `start landing`, `новый проект` → `/workflow-start`
- Продолжить: `продолжить запуск`, `resume workflow`, `погнали дальше` → `/workflow-resume`
- Статус: `покажи статус`, `workflow status`, `что готово` → `/workflow-status`

Этапы:

- Research: `сделай ресерч`, `исследуй конкурентов`, `run research` → `/research`
- PRD: `напиши prd`, `сформируй требования`, `generate prd` → `/prd`
- IA: `сделай sitemap`, `нарисуй user flow`, `design architecture` → `/ia`
- Design: `подготовь дизайн-бриф`, `создай дизайн`, `make design brief` → `/design`
- Screens: `сгенерируй спецификацию экранов`, `создай экраны`, `generate screens` → `/screens`
- Prototype: `создай прототип`, `сделай transition map`, `make prototype` → `/prototype`
- Copywriting: `напиши тексты`, `сделай copy deck`, `write landing copy` → `/copy`
- Frontend: `напиши код`, `сверстай лендинг`, `implement frontend` → `/frontend`
- Visual Diff: `сравни с референсом`, `проверь скриншоты`, `visual diff` → `/visual-diff`
- Test Bench: `запусти тест-бенч`, `проверь воронку`, `run test bench` → `/test-bench`
- QA Review: `проверь качество`, `запусти qa`, `run qa review` → `/qa`
- Release: `подготовь релиз`, `создай релиз-ноутс`, `release now` → `/release`
- Notion: `выложи в ноушен`, `опубликуй в notion`, `publish to notion` → `/notion-publish`
