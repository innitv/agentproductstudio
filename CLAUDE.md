...
Правила, которые нужно помнить сразу (полный текст — skill `recursive-brief`, шаг 3.3):

- **Режется глубина проработки, не защита.** Approval gates, run ledger, Anti-AI-Slop, Russian Publication Gate и статусы одинаковы на любом масштабе; `00-intake` и `11-qa` входят во все. Масштаб — НЕ способ обойти гейт.
- **Масштаб выводится из утверждённого плана работ, а не выбирается категорией.** На `00-intake` человеку показывается список работ и правится им; масштаб — следствие правки. Молчаливый выбор без показанного плана — `process_deviation`.
- **Не уверен — бери `full`.** Занижение «на глаз» запрещено, задним числом — только через `process_deviation`.
- **Пропущенные по масштабу стадии пишутся в `stage-gate-ledger.md` как `skipped_by_scale`**: молчаливый пропуск неотличим от забытой стадии, проверяется машинно.
...
Показ пишется **командой**: `yarn workflow:human-review <run-dir> <7.5|8.5a|8.5b> --notes "…"`. Руками эту строку не поставили ни в одном из 10 прогонов (аудит 2026-08-17). Все три точки проверяет `yarn workflow:validate` (условие для `7.5` и законный выход через `process_deviation` — `claude-operating-rules.md` §6.1).
...
**Два гейта человека, которые нельзя закрыть фразой «делай дальше»** (норма, прецеденты и замеры — `claude-operating-rules.md` §6.1):

- **носитель экранов выбирает человек на `00-intake`** — макеты в Figma или сразу код; `04-design` не вправе решить за него. Выбрана Figma — макеты показываются человеку до `08-frontend` ссылкой и скриншотами. «Макеты готовы» гейт не закрывает — закрывает показ;
- **два живых просмотра внутри `08-frontend`** (8.5a витрина, 8.5b страница). Машинная приёмка их не заменяет: на `a3-shadcn` восемь дефектов прошли все четыре оси зелёными и были найдены человеком за минуту, пять из них — уровня компонента.
...
Решение владельца продукта от 2026-07-27. Для нового product UI компоненты **берутся из shadcn/ui**, а не проектируются с нуля (`yarn shadcn add <component>`, команды — в `COMMANDS.md`). Код копируется в проект и принадлежит ему: править можно и нужно, обновлений «сверху» нет.
...
**Механика библиотеки — навык `shadcn-library`** (в `skills:` у `design`, `design-generator`, `frontend`, `qa-review`): чего в реестре нет, известные грабли, Figma-кит и его ограничения, а также §1.1 — разграничение четырёх одноимённых инструментов (навык, MCP-сервер `shadcn`, два вендорских навыка). **Грабли пишутся в навык, а не в этот индекс:** индекс грузится в каждый контекст и от прогона к прогону так уже удваивался (аудит 2026-07-30).
...
- `approval-gate` — перед любым внешним действием: Notion, Figma, git, deploy, секреты, удаление, provider call. Молчаливый пропуск approval-вопроса запрещён.
- `recursive-brief` — `00-intake`: три прохода до research, опрос про образец и утверждаемый план работ. Молчаливый выбор масштаба запрещён.
- `run-ledger` — ведение ledger после каждого этапа.
