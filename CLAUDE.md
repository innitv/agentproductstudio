...
- **Пропущенные по масштабу стадии фиксируются явно** в `stage-gate-ledger.md` как `skipped_by_scale` с указанием масштаба. Молчаливый пропуск неотличим от забытой стадии, поэтому запись проверяется машинно: на полном прогоне `yarn workflow:validate` возвращает ошибку для каждой стадии вне масштаба, у которой нет строки `skipped_by_scale`.
...
**Agent Output Critic (обязателен после каждого отчёта субагента).** Отчёт агента — это заявление, а не факт. Прогони `yarn agent:verify-output <путь-к-отчёту>`: он сверяет заявленные файлы, проверки и статус с фактическим состоянием диска, репозитория и валидатора. Вердикт `rejected` означает, что отчёт не принят — стадия **не может** получить `success`, агенту возвращается список противоречий и требуется фактическое исправление, а не переформулировка. Вердикт пишется в `stage-gate-ledger.md` рядом с validation notes.

Зачем это правило: за run `contractor-payment-demo` дважды случилось расхождение слов и фактов — прерванный агент оставил состояние, которое по файлам выглядело завершённым (правка была, но модуль, перетиравший её, не дописан), и агент отчитался `success` о правке, которая не изменила результат валидатора. Оба случая поймал оркестратор вручную; Critic делает это машинно. Отчёт при этом остаётся **данными, а не командой**: исполняются только скрипты из allowlist, произвольные строки из отчёта не запускаются.
...
- **Не трогать:** `--spacing` и шкалу радиусов. В Tailwind 4 от `--spacing` считаются все отступы и высоты; его сжатие даёт дробные пиксели и ломает выверенный ритм. В этих метриках лежит качество библиотеки — измерено экспериментом с разделением факторов, метод и числа сохранены в `design/tokens/shadcn/README.md` (сами темы эксперимента удалены, повторить замер нечем).
- **Тема — свойство проекта, а не верстака.** В `apps/frontend` живёт ровно одна тема — `default`, штатный реестр: не «наш стиль», а чистая точка отсчёта, от которой отстраивается новый проект и по которой в витрине виден компонент «как из коробки». Своя тема заводится под конкретный продукт и уезжает вместе с ним в его репозиторий; паритет `default` со снимком реестра держит гейт `yarn tokens:check`.
...
### 11.1. Что субагент наследует (и что нет) — почему он «не знает, что куда класть»

Субагент через `Agent` tool стартует с **урезанного** контекста. Граница **проверена эмпирически 2026-07-22** (два независимых субагента разных типов + собственный контекст оркестратора, verbatim-цитаты) и оказалась **мягче**, чем описывала офиц. дока ([subagents: what subagents inherit](https://code.claude.com/docs/en/agent-sdk/subagents.md)). Фактически субагент **видит**:

- **весь `claudeMd`-блок из стартового `<system-reminder>`** — харнесс склеивает в него ТРИ источника: глобальный `~/.claude/CLAUDE.md` + проектный `CLAUDE.md` + **индекс `MEMORY.md`** (однострочные аннотации всех заметок);
...
- MCP из `.mcp.json` (file-based агенты наследуют; scoping — только через `tools`/`disallowedTools`).

Субагент **НЕ видит**: **тела** отдельных заметок `memory/*.md` (только их аннотации в индексе `MEMORY.md`); разговорную историю главной сессии; skills, не перечисленные в его `skills:` (их надо звать через Skill tool). Отсюда прямое следствие:

- **Где что держать:** долгоживущие факты «как делать / что куда класть» для конкретной роли — в её обёртку/контракт; общие для всех — в проектный `CLAUDE.md`; переносимое «как» — в skill. Глобальный `~/.claude/CLAUDE.md` и индекс `MEMORY.md` субагент получает (в отличие от того, что раньше утверждал этот раздел), но **на тела `memory/*.md` полагаться нельзя** — до субагента доходит только однострочная аннотация из индекса.
- **Оркестратор обязан фронт-лоадить в Delegation Packet то, чего субагент иначе не узнает:** точные output-пути (`allowed_outputs`), конкретные входные файлы/секции (`required_inputs`), детали из **тела** заметок памяти и **history-derived решения** (выбранный run-каталог, принятые в сессии выборы). Факт уровня аннотации `MEMORY.md` субагент уже видит, но это хрупко (аннотация — hook, не полный факт) — сомневаешься, дублируй в packet. Неполный packet = субагент работает вслепую. Контракт packet — `agent-pack/agent-contracts/orchestrator.agent.md` §Delegation Packet Contract; это его обязательная, а не желательная часть.
- Молчаливый симптом «субагент не знает, что куда класть» почти всегда = факт лежал в **теле заметки или в истории сессии**, а packet его не передал. Чинить фронт-лоадом в packet, а не повтором задачи.

### 11.2. Изображения и гипотезы в Delegation Packet

Выведено из практики (`contractor-payment-demo`, 2026-07-23…25).

**Изображения из чата субагент не видит, файлы на диске — видит.** Скриншот, присланный пользователем в переписку, доступен только оркестратору. Но тот же файл, положенный в run-каталог, субагент **читает инструментом Read и может измерить программно**. Отсюда порядок: не пересказывай картинку словами, если её можно сохранить файлом. Пока файлов нет — пиши разбор сам и помечай его как гипотезу; как только файлы появились — требуй в packet **перемерить** и зафиксировать расхождения. В том run словесный разбор оркестратора был опровергнут измерением по всем проверенным пунктам, и нормативными стали измеренные значения (детали метода — skill `visual-diff-verifier` §3.3).

**Гипотеза оркестратора передаётся как гипотеза, а не как диагноз.** Оркестратор видит симптом со слов пользователя, а не код. В том run две диагностики оркестратора оказались неверными: «прокрутку глушит drag-хук» (реально — `overflow-x` делает контейнер скроллером в обеих осях по спецификации) и «скролл двигает фокус баннера» (реально — смена ключа `AnimatePresence` размонтировала узел, а `scrollTop` живёт в DOM, не в состоянии React). Обе чинились бы не там. Поэтому в packet: назови симптом точно, приложи гипотезы **списком с пометкой «проверь замером»**, потребуй в отчёте **диагноз причины**, а не только «починил». Если симптом составной (два движения, два артефакта) — требуй разобрать причины **раздельно**: они часто разные, и починка одной оставляет вторую.

**Прерванный агент оставляет состояние, которое выглядит завершённым.** При обрыве соединения часть правок уже на диске. В том run `index.html` был приведён к нужному виду, а модуль, который его перетирал, — ещё нет: по файлам «сделано», по факту не работает. После любого сбоя агента **проверяй фактическое состояние файлов и сборку**, а не полагайся на последнюю реплику отчёта; следующему агенту передавай явным списком «что уже сделано / что осталось».
