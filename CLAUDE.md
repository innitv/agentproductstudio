...
- **`shadow-xs` зашит во ВСЕ интерактивные примитивы реестра** — `Button` (варианты `default` и `outline`), `Input`, `Textarea`, `Select`. Токеном не снимается (см. пункт выше про тени-константы). Если в макете теней нет, снимать надо **сразу по всей группе** правилом уровня темы, а не по одному элементу за замечание: иначе каждый следующий добавленный компонент приносит тень заново. Измерено 2026-07-29 на А3 — тени ловились глазами в два захода, сначала у кнопок, потом у полей.
- 🔴 **Тёмной темы в проекте нет — и завязываться на системную нельзя.** В Tailwind 4 вариант `dark:` по умолчанию слушает `prefers-color-scheme`, а компоненты реестра несут тёмные варианты прямо в классах (`dark:bg-input/30` у `Input` и у `Button variant=outline`). На машине с тёмной системной темой они срабатывают и красят поля и вторичные кнопки серой подложкой, хотя тёмных токенов в проекте не объявлено. Поэтому в `apps/frontend/src/styles.css` вариант привязан к классу: `@custom-variant dark (&:where(.dark, .dark *))`. Правило шире одного бага: **пока тёмная тема не заложена осознанно, ничего не подменяем по системной настройке и внимания на неё не обращаем.** Понадобится тёмная тема — она заводится как тема со своими токенами и своим классом, а не наследуется от ОС. Цена ошибки измерена: дефект прошёл `vr:test`, `test-storybook`, `qa:mobile` и axe — контейнер Playwright и CI стартуют со светлой системной темой, и увидеть его машинно было нельзя (нашёл пользователь глазами, 2026-07-29).
...
- Индексы состава компонентов **кода не заводить**: `apps/frontend/src/components/shadcn/` читается напрямую, второй источник правды о нём неизбежно разъедется (прецедент — `token-map.md`, описывавший 28% реальности). Запрет про код; индекс чужого **Figma-файла** — не он: содержимое Figma из кода не выводится, см. следующий пункт.
- **Макеты в Figma — по заинжектенному киту, а не с нуля.** `design/figma/shadcn-ui-community/` (2026-07-28): 172 компонента и сета с Node ID, ключами и матрицами вариантов, семантические переменные light/dark, 14 125 иконок. Повторно не ингестить. Два ограничения, которые фиксируются в `design-brief.md` как явное решение: кит **не опубликован как библиотека** — макеты собираются внутри его файла, импорт по ключу снаружи не работает (проверено негативным контролем); его цветовая база `neutral` против нашей `slate` — макет по киту как есть не равен коду попиксельно. Разбор и способы — `foundation.md` кита. Источником правды о составе и поведении компонентов остаётся код: кит отвечает на вопрос «что где лежит в Figma».
...
- `rule-placement` — **при любом изменении правила, нормы или гейта студии**: карта адресов (индекс / operating rules / контракт агента / обёртка / хук / skill / тест / плагин), чек-лист связности «кто исполняет — знает, кто проверяет — знает, кто ведёт запись — знает» и обязательный отчёт в чат о том, куда правило разложено и что осталось непокрытым. Механическая часть — грепнуть маркер правила по репозиторию: **результат в один файл значит, что правило знает только тот, кто его записал**. Прецеденты, на которых заведён: знание о библиотеке, осевшее в продуктовом CSS, и машинная проверка `human_review`, о которой не знал ни один исполнитель.
