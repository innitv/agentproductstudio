...
Нативные Claude-обёртки — в `.claude/agents/` (вызываются главной сессией через `Task` tool, `subagent_type` = имя): research, prd, ia, design, design-generator, copywriting, prototype, frontend, test-bench, qa-review, release, notion-publisher. Оркестратор — это сама главная сессия (`.claude/agents/orchestrator.md` — её чек-лист), не спавни его как субагента. Детальные контракты — `agent-pack/agents/*.agent.md`. Skills — в `.claude/skills/` (детально — `agent-pack/skills/`).
