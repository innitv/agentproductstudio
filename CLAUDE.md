...
- **Не трогать:** `--spacing` и шкалу радиусов. В Tailwind 4 от `--spacing` считаются все отступы и высоты; его сжатие даёт дробные пиксели и ломает выверенный ритм. В этих метриках лежит качество библиотеки — измерено на паре `branded`/`calm`.
