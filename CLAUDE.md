...
7.5. 🔴 **Показ макетов человеку** — если носитель экранов Figma: ссылка, скриншоты, утверждение. Вёрстка не стартует без «да».
8. Frontend: реализация, состояния, адаптивность, analytics hooks. Внутри стадии два обязательных гейта человека:
   - 8.5a. 🔴 **Показ витрины** — компоненты и все состояния собраны, страницы ещё нет. Оркестратор поднимает Storybook и ждёт замечаний. Здесь ловятся дефекты «кирпичей»: цвет, гарнитура, тени, размеры, состояния.
   - 8.5b. 🔴 **Показ страницы** — композиция собрана. Оркестратор поднимает dev-сервер, даёт роут и обе точки адаптива. Здесь ловятся дефекты «дома»: ритм, зазоры, порядок секций, поведение на 390. Только после этого идут `09` и `11`.

Показ пишется **командой**: `yarn workflow:human-review <run-dir> <7.5|8.5a|8.5b> --notes "…"`. Руками эту строку не поставили ни в одном из 10 прогонов (аудит 2026-08-17).
...
**Два гейта человека, которые нельзя закрыть фразой «делай дальше»** (полный текст и прецеденты — `claude-operating-rules.md` §6.1):

- **носитель экранов выбирает человек на `00-intake`**: макеты в Figma или сразу код. Стадия `04-design` не вправе решить это за него; глобальное правило владельца — «макеты в Figma, если не сказано иначе», и «иначе» произносит человек. Кит shadcn в Figma для этого и заинжектен. **Выбрана Figma — макеты собираются на `06-screens` и показываются человеку до `08-frontend`**: ссылка на файл и node, скриншоты, вопрос «утверждаем или правим». Вёрстка ждёт «да». Сообщение «макеты готовы» гейт не закрывает — закрывает показ;
- **два живых просмотра внутри `08-frontend`** (пункты 8.5a и 8.5b выше): витрина до сборки страницы, страница после. Измерено на `a3-shadcn` 2026-07-29: восемь дефектов прошли `vr:test` 95/0, `test-storybook` 64/0, `qa:mobile` 5/0/0 и axe 0 нарушений — и были найдены человеком за минуту. **Пять из восьми были дефектами уровня компонента** и существовали в витрине до того, как из этих компонентов собрали страницу в 5714 px; показ был один и в самом конце, поэтому композиция строилась из непроверенных кирпичей.
...
Решение владельца продукта от 2026-07-27. Для нового product UI компоненты **берутся из shadcn/ui**, а не проектируются с нуля. CLI стоит как devDependency: `yarn shadcn add <component>` (команды из официальной документации в этой среде неприменимы, детали в `COMMANDS.md`). Код копируется в `apps/frontend/src/components/shadcn/` и принадлежит проекту — править его можно и нужно, обновлений «сверху» нет.

Три решения уровня проекта:

- **Менять смело:** цвет, гарнитуру, кольцо фокуса. **Не трогать:** `--spacing` и шкалу радиусов — в Tailwind 4 от `--spacing` считаются все отступы и высоты, и именно в этих метриках лежит качество библиотеки (замер — `design/tokens/shadcn/README.md`).
- **Тема — свойство продукта, а не верстака.** В `apps/frontend` живёт ровно одна тема `default` как чистая точка отсчёта; своя тема заводится под продукт и уезжает с ним в его репозиторий. Паритет держит гейт `yarn tokens:check`.
- 🔴 **Тёмной темы нет, и завязываться на системную нельзя** — ни в коде, ни вниманием. Компоненты реестра несут тёмные варианты в классах, поэтому `dark:` привязан к классу в `apps/frontend/src/styles.css`; удаление той строки возвращает дефект, который прошёл всю машинную приёмку. Проверяется `yarn workflow:test-studio-hygiene`.

**Механика библиотеки — навык `shadcn-library`** (в `skills:` у `design`, `design-generator`, `frontend`, `qa-review`; вызывается по имени): чего в реестре нет, `shadow-xs` во всех интерактивных примитивах, порталы вне контейнера темы, полный разбор тёмной темы, Figma-кит `design/figma/shadcn-ui-community/` и его два ограничения, запрет на индексы состава компонентов кода. Там же §1.1 — разграничение четырёх одноимённых инструментов: сам навык, **MCP-сервер `shadcn`** (поиск по 264 реестрам, чтение кода компонента до установки), вендорские навыки `shadcn` и `migrate-radix-to-base` (ставятся установщиком навыков `shadcn/ui`, команда — в `COMMANDS.md`). Каждое правило там стоило прохода сборки или дефекта — **грабли пишутся в навык, а не в этот индекс**: индекс грузится в каждый контекст, и от прогона к прогону он так уже удваивался (аудит 2026-07-30).
...
- `rule-placement` — **при любом изменении правила, нормы или гейта студии**: карта адресов, чек-лист связности «кто исполняет / проверяет / ведёт запись — знает?» и отчёт в чат о раскладке. Механическая часть — грепнуть маркер: **выдача в один файл значит, что правило знает только автор**.
- `run-retrospective` — разбор завершённого run по его артефактам (`yarn workflow:retro`, `/retro`): пять метрик процесса и правило «одна находка — одно из трёх решений». Разбирает хронику ОДНОГО run; расхождения устройства системы — это `/subsystem-audit:audit`.
- `/subsystem-audit:audit` — аудит подсистемы («проверь/оцени/улучши X», сверка с best practice): верификация находок первоисточником, GitHub-сравнение по реальным URL, эвристики против ложных находок. Junction-плагин.
- `/ui-craft:build` и `/ui-craft:reference-check` — переносимое ремесло интерфейса: как реализовать качественно и чем доказать соответствие образцу. Junction-плагин; основу интерфейса он не выбирает — это §6.1, а `landing-builder`/`visual-diff-verifier` остаются проектными.

Текущее покрытие стадий skills: `yarn workflow:skills`.

### 11.1. Что субагент видит и что обязан дать ему packet

Субагент стартует с урезанным контекстом: **видит** `claudeMd`-блок (глобальный + проектный `CLAUDE.md` + индекс `MEMORY.md`), свою обёртку, `SKILL.md` из своего `skills:`, prompt из packet и MCP; **не видит** тела заметок `memory/*.md`, историю сессии, изображения из чата и навыки вне своего `skills:`. Отсюда: фронт-лоадить в packet точные пути и решения разговора, картинку класть файлом, гипотезу передавать как гипотезу, после сбоя агента проверять факт, а не отчёт.
