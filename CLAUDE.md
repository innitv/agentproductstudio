...
**Механика библиотеки — навык `shadcn-library`** (в `skills:` у `design`, `design-generator`, `frontend`, `qa-review`; вызывается по имени): чего в реестре нет, `shadow-xs` во всех интерактивных примитивах, порталы вне контейнера темы, полный разбор тёмной темы, Figma-кит `design/figma/shadcn-ui-community/` и его два ограничения, запрет на индексы состава компонентов кода. Каждое правило там стоило прохода сборки или дефекта — **грабли пишутся в навык, а не в этот индекс**: индекс грузится в каждый контекст, и от прогона к прогону он так уже удваивался (аудит 2026-07-30).
