...
- `orchestrator` (главная сессия Claude Code) владеет финальным результатом; специалисты вызываются как bounded capabilities через `Task` tool с `subagent_type`.
...
Детальные контракты специалистов (frontmatter, required inputs/outputs, gates, output contract) — в `agent-pack/agents/*.agent.md`. Для повторяющихся технических действий используй skills из `.claude/skills/` (детально — `agent-pack/skills/`).
