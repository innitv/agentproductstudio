...
Субагент через `Agent` tool стартует с **чистого контекста**. Проверено по офиц. доке ([subagents: what subagents inherit](https://code.claude.com/docs/en/agent-sdk/subagents.md)). Он видит **только**:

- **этот `CLAUDE.md`** (project-level) — авто-инъекция;
- свою обёртку `.claude/agents/<имя>.md` (системный промпт);
- **строку prompt из Delegation Packet** — и больше ничего;
- MCP из `.mcp.json` (file-based агенты наследуют; scoping — только через `tools`/`disallowedTools`).

Субагент **НЕ видит**: авто-память `memory/MEMORY.md` и все `memory/*.md`; глобальный `~/.claude/CLAUDE.md`; разговорную историю главной сессии; skills, не перечисленные в его `skills:` (их надо звать через Skill tool). Отсюда прямое следствие:

- **Знание, нужное субагенту, нельзя держать только в памяти или глобальном CLAUDE.md — он их не получит.** Долгоживущие факты «как делать / что куда класть» для конкретной роли идут в её обёртку/контракт; общие для всех — в этот файл (авто-наследуется); переносимое «как» — в skill.
- **Оркестратор обязан фронт-лоадить в Delegation Packet то, чего субагент иначе не узнает:** точные output-пути (`allowed_outputs`), конкретные входные файлы/секции (`required_inputs`) и релевантные факты из памяти (слаги, лимиты, выбранные решения). Неполный packet = субагент работает вслепую. Контракт packet — `agent-pack/agent-contracts/orchestrator.agent.md` §Delegation Packet Contract; это его обязательная, а не желательная часть.
- Молчаливый симптом «субагент не знает, что куда класть» почти всегда = факт лежал в памяти/истории, а packet его не передал. Чинить фронт-лоадом в packet и переносом факта на видимый субагенту слой, а не повтором задачи.
