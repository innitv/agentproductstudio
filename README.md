...
- **Что искать до установки** — MCP-сервер `shadcn`: 264 реестра, поиск, чтение кода компонента и примеров до того, как он приедет в репозиторий. Метки `Free`/`Pro` в выдаче MCP нет, а платных большинство — проверять на странице блока; платный не вскрывается, приём воспроизводится с нуля. После любой установки — `git status` и `git diff`: `--overwrite` стирает проектные правки молча. Порядок целиком — `shadcn-library` §1.1 и §4.5.
- **Приёмка машинная.** «Похоже» и просмотренный человеком скриншот приёмкой не считаются: для `product_ui|frontend` нужны три вердикта — `yarn vr:test` (пиксельная регрессия витрины), `yarn test-storybook` (поведение и a11y), `yarn qa:mobile` (профиль устройства). `vr:test`/`vr:update` исполняются только внутри пиннутого Docker-образа Playwright: эталон, снятый на Windows-хосте, на Linux не сравнивается, а молча создаёт новый файл. **Для многостраничного surface добавляется четвёртая ось — разница МЕЖДУ страницами** (`claude-operating-rules.md` → Machine Acceptance Gate): три оси выше проверяют страницу саму по себе, и разъехавшийся общий каркас проходит их зелёным.
...
Skills подключаются автоматически по описанию — своих команд у них нет. Кросс-стадийные: `approval-gate` (любое внешнее действие), `recursive-brief` (intake), `run-ledger` (журнал запуска), `anti-ai-slop` (перед записью research/PRD/copy и публикацией), `selective-commit`, `outputs-cleanup`, `run-retrospective` (разбор завершённого run, команда `/retro`), `rule-placement` (изменил правило студии — разложи по адресам: карта «библиотека → индекс, процесс → operating rules, обязанность роли → контракт и обёртка, шаги → skill, напоминание → хук, „X совпадает с Y“ → тест», чек-лист связности и отчёт со строкой «не покрыто»). Покрытие стадий: `yarn workflow:skills`.
...
| `.mcp.json` | MCP-серверы: figma, figmaDesktop, notion, tavily, playwright, github, gitlab, lazyweb, **shadcn** (поиск по 264 реестрам компонентов — порядок работы в `shadcn-library` §1.1) |
