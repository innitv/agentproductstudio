 ...
 *
 * ─── ЗАЧЕМ ОТДЕЛЬНЫЙ СБОРЩИК ────────────────────────────────────────────────
 * Токены A3 (`design/tokens/primitive|semantic|component`) собирает
 * `tooling/tokens/build-tokens.mjs` со своим baseline-гейтом. Смешивать две
 * системы в одном конвейере нельзя: у них разные baseline и разная политика
 * изменений. Поэтому shadcn живёт в отдельной коллекции
 * `design/tokens/shadcn/` и собирается этим скриптом. `yarn tokens:build`
 * (A3) остаётся нетронутым, его паритет обязан оставаться нулевым.
 *
 * ─── ЧТО ГЕНЕРИРУЕТСЯ ───────────────────────────────────────────────────────
 ...
 * `rounded-lg` во всём приложении начинает считаться от `--radius`. В этом
 * репозитории `rounded-*` уже используют `LandingView.tsx` и `ConsoleView.tsx`,
 * то есть глобальная подмена изменила бы чужие экраны.
 *
...
const lines = [
  ...
  " *      виден фон именно html; без этой строки там проступает брендовый синий",
  " *      фон лендинга из :root файла styles.css.",
  " * Никакие другие свойства на html не ставятся: тема не должна протекать",
  " * на соседние страницы A3.",
  " */",
...
if (process.argv.includes("--check")) {
  ...
  if (actual !== css) {
    fail(`${path.relative(ROOT, OUTPUT)} устарел. Выполни: yarn tokens:build:shadcn`);
  }
