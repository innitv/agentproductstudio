...
Исторические ledger-файлы отражают состояние на дату run. Если в них встречаются старые route/path references, не используй их как текущие правила архитектуры; сверяйся с `AGENTS.md`, этим документом, `docs/architecture/git-workflow.md` и актуальными app configs.
...
4. Исторические artifacts в `outputs/**`, `research/projects/**` не переписываются ради актуализации правил. Новые правила живут в `AGENTS.md`, `README.md`, `docs/architecture/**`, agent docs и runtime configs.
