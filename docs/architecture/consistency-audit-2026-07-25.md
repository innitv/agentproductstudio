...
2. **Расширение `audit-docs.mjs`** на `docs/**`, `agent-pack/**`, `.claude/**` и на markdown-ссылки `[](...)`, либо lychee в CI. Закрывает класс P2-1/P1-6/P1-8. — **Реализовано 2026-07-25, см. 9.**
...
- ~~**Расширение `audit-docs.mjs`**~~ — сделано третьим заходом, см. раздел 9. Показательно, что все три документационные находки предыдущего захода (P1-6, P1-8, P2-1) лежали ровно в непокрытой зоне.
...
## 9. Расширение `audit-docs.mjs` — 2026-07-25 (третий заход)

Предложение 6.2 реализовано. Мотив прямой: все документационные находки предыдущих заходов (P1-6, P1-8, P2-1) лежали в зоне, которую скрипт не смотрел — он проверял backtick-пути ровно в трёх корневых файлах.

### 9.1. Зона покрытия

Было: `README.md`, `CLAUDE.md`, `AGENTS.md`. Стало: те же плюс `COMMANDS.md`, `docs/**`, `agent-pack/**`, `.claude/**`, `plugins/**`, `outputs/README.md`, `research/README.md` — **162 markdown-файла**. `plugins/**` добавлен сверх предложения: плагины `figma-ds` и `subsystem-audit` — такая же нормативная документация, ссылающаяся на пути репозитория.

### 9.2. Проверки и класс ошибок, который ловит каждая

| # | Проверка | Класс ошибок |
|---|---|---|
| 1 | Обязательные корневые документы существуют | Была; расширена до `COMMANDS.md` |
| 2 | Пути в backticks существуют | Мёртвая запись про удалённый каталог (P1-8, P2-2), протухший ручной список |
| 3 | Markdown-ссылки `[текст](путь)` ведут на существующий файл | Переименование/перенос документа, оставившее ссылку |
| 4 | Упомянутая `yarn <script>` есть в `package.json` | Переименован или удалён скрипт, документация осталась |
| 5 | Каждый скрипт `package.json` описан в `COMMANDS.md` | Добавлен скрипт — справочник, который сам себя называет полным, устарел (закрепляет правку 8.4) |
| 6 | Каждый MCP-сервер из `.mcp.json` упомянут в `README.md` и `CLAUDE.md` | Ровно класс P2-3: добавлен `figmaDesktop`, списки в документах не обновились |
| 7 | Самопроверка: не меньше 100 файлов и 250 путей | Сломанный обход дерева даёт «зелено» на пустоте — тот же приём, что `assert` на числе скелетов в `test-agent-output-skeletons` |

### 9.3. Правила против ложных срабатываний

Скрипт стоит в `pre-commit` (`qa:quick`), поэтому ложное срабатывание блокирует работу. Отбор кандидатов:

- содержимое fenced code blocks не читается (нумерация строк сохраняется);
- отбрасываются кандидаты с `<>`, `{}`, `$`, `*`, `|`, `,`, пробелами и заканчивающиеся на `...` — это плейсхолдеры и glob-шаблоны;
- `путь.ts:120` и `путь.md:12-34` нормализуются в путь (в отчётах студии это ссылка на строку);
- внешние URL, `mailto:`, `tel:`, якоря `#...` не проверяются;
- markdown-ссылка валидна, если резолвится **либо** относительно файла, **либо** относительно корня репозитория — в проекте живут оба стиля, и требование одного из них дало бы 20 ложных срабатываний в `agent-pack/agent-contracts/**`;
- цель ссылки проверяется только если она ASCII-путь: `[текст](путь)` в прозе — это пример синтаксиса, а не ссылка;
- **датированные снимки** `*-YYYY-MM-DD.md` (7 файлов в `docs/architecture/`, включая этот) не сканируются на пути: отчёт аудита законно цитирует уже удалённые каталоги как находку;
- строка с маркером `docs-audit-ignore` в HTML-комментарии пропускается; файл с `docs-audit-ignore-file` — целиком.

Маркер понадобился ровно один раз: `repo-map.md:47` — фраза «ни ветки, ни каталога `apps/a3pay-demo` никогда не существовало». Машина не понимает отрицания, поэтому исключение сделано явным и с причиной в самом комментарии.

### 9.4. Найденные битые ссылки

**Реальных битых ссылок в живой документации не найдено.** Это ожидаемо: заходы 7 и 8 чинили ровно этот класс вручную, и расширенный скрипт подтвердил результат машинно. Единственное срабатывание — упомянутая выше негативная формулировка в `repo-map.md:47`, ложная по смыслу.

Проверено 419 путей в backticks, 77 markdown-ссылок, 55 `yarn`-команд, 79 скриптов `package.json`, 8 MCP-серверов.

### 9.5. Негативный контроль

Выполнен в двух подходах, оба с возвратом состояния.

1. Временные файлы в трёх зонах покрытия (`docs/architecture/`, `agent-pack/`, `.claude/`) с битым путём, битой markdown-ссылкой и несуществующей `yarn`-командой — плюс контрольная строка с плейсхолдерами, внешним URL и якорем, которая падать не должна. Скрипт вернул код 1 и четыре сообщения с `файл:строка`, ложных срабатываний на контрольной строке нет.
```
Documentation audit failed:
- docs/architecture/tmp-negative-control.md:3: путь не существует: `runtime/typescript/no-such-module.ts`
- docs/architecture/tmp-negative-control.md:4: ссылка ведёт в никуда: (../../agent-pack/workflows/no-such-workflow.md)
- .claude/tmp-negative-control.md:3: путь не существует: `.claude/agents/no-such-agent.md`
- agent-pack/tmp-negative-control.md:3: команда `yarn workflow:no-such-command` отсутствует в package.json
EXIT CODE: 1
```

2. Проверки 5 и 6 — подстановкой фиктивного скрипта в `package.json` и фиктивного сервера в `.mcp.json`; сработали обе, тремя сообщениями (COMMANDS.md — скрипт не описан в справочнике; README.md и CLAUDE.md — MCP-сервер не упомянут). Проверка 7 — сокращением списка сканируемых каталогов: «самопроверка аудита: найдено 15 md-файлов и 157 путей (ожидалось не меньше 100 и 250)».

После удаления временных файлов и возврата конфигов — `Documentation audit passed`, код 0.

### 9.6. Стоимость и что осознанно не покрыто

Прогон — **~100 мс** (три замера: 104/103/102 мс), `yarn docs:audit` целиком — 0.19 s. На `pre-commit` не сказывается.

Не покрыто намеренно:

- **Якоря внутри markdown** (`file.md#заголовок`): нормализация кириллических заголовков в anchor неоднозначна, риск ложных срабатываний выше пользы.
- **Пути без префикса-каталога** (`workflow.manifest.ts` без `runtime/typescript/`): без контекста не резолвятся; расширять отбор — значит гадать.
- **Датированные снимки**: см. 9.3, это осознанная граница, а не пробел.
- **`Bash(yarn deploy:*)` в `.claude/settings.json`**: проверку permissions против `package.json` не добавляли — она бы сразу упала на этом мёртвом permission, а правка `settings.json` остаётся за пользователем.
- **Парность `description` у skills**: как и раньше, машиной не проверяется — runtime явно разрешает обёртке быть короче.
- **CI**: скрипт по-прежнему только локальный, workflow в `.github/` нет (см. раздел 5).
...
- 2026-07-25 — реализовано предложение 6.2: `tooling/scripts/audit-docs.mjs` расширен с 3 файлов до 162 и с одной проверки до семи (пути, markdown-ссылки, `yarn`-команды в обе стороны, MCP-серверы, самопроверка сканера). Детали — раздел 9. Git-операции не выполнялись.
