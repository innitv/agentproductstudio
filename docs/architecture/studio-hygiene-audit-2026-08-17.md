...
## 5.1. Что закрыто в тот же день (коммит `113bbdd`)

Владелец распорядился делать всё. Сделано пятью этапами, каждый с тестом и
негативным контролем:

| Находка | Как закрыта |
|---|---|
| P0 гейты | Модуль `human-review-gate.ts` + команда `yarn workflow:human-review`; тест сверяет формат записи с регуляркой валидатора; норма разложена по 10 адресам |
| P0, доп. | 🔴 Валидатор научен распознавать `process_deviation: human_review <точка>` — до этого он предлагал этот ход в тексте ошибки, но сам его не принимал. Найдено при попытке честно закрыть прогон портфолио |
| P1 индекс | 34 769 → 33 355 символов, порог 35 000 → 33 400. Вынесено: §11.1 (полный текст был в `delegation-lessons.md`), описания четырёх навыков (процедура — в самих навыках) |
| P2 doctor | `collectStudioHygieneFindings` вызывается из `workflow:doctor`, уровень `warning` |
| P2 CLI | `--help` не создаёт прогон; слаг проверяется на входе (`--slug`); `research:lint` требует путь (exit 2); `figma:audit` получил `default_system` и `--dry-run`; обе команды вошли в агрегатор через `test-cli-arg-contracts` |
| P2 мусор | Удалены `outputs/archive/help/*`, `outputs/archive/3/*`, пустая `design/figma/product-agent-studio-deck/` |

**Не закрыто сознательно:**

- `outputs/products/` оставлен — описан в нормах как legacy-зона, удаление потребовало бы правки трёх документов ради пустой папки;
- `outputs/temp/ai-contest-pitch/` оставлен — заявка на конкурс и питч-скрипт, 100 КБ содержательного текста, а не мусор;
- **девять прогонов из десяти остаются с долгом гейтов.** Закрыт только прогон портфолио: запись 8.5b по факту показов и `process_deviation` по 8.5a (витрину отдельно не показывали, владелец правил по собранной странице). Остальные закрыть задним числом значило бы соврать в ledger.

Приёмка после закрытия: `typecheck`, `validate:config`, `docs:audit`, `workflow:doctor` (14 проверок), `workflow:test-agentic` — 33 команды зелёные.
