...
Коммиты подписываются одной и той же парой имя/почта, и она зафиксирована в
`.claude/git-identity` — одна строка, комментарии `#` игнорируются. Хук
`.claude/hooks/guard-bash.mjs` сверяет с ней `git config user.email` перед
`git push` и `git commit`; расхождение блокирует команду, осознанный обход —
`CLAUDE_ALLOW_IDENTITY_MISMATCH=1`. Файла нет — проверка молчит, это fail-open
по устройству.

🔴 **Повод (2026-09-04).** У владельца два аккаунта GitHub. Push шёл под
рабочим (`innitv`, токен в диспетчере учётных данных Windows), а коммиты были
подписаны почтой второго (`ignatov@a-3.ru`), привязанной к аккаунту
`ignatovivin`. GitHub сопоставляет коммит с профилем по e-mail автора, поэтому
в списке коммитов стоял не тот пользователь. Расхождение заметил человек, хотя
оба факта были доступны машине.

Менять значение в `.claude/git-identity` — сознательное решение, а не правка
по ходу: оно определяет, за кем числится история. Служебный адрес вида
`<id>+<login>@users.noreply.github.com` предпочтительнее личной почты — он
всегда сопоставляется верно и не публикует адрес.

Проверка закреплена тестом `yarn workflow:test-guard-bash-identity`: держит
блокировку при расхождении, тишину при совпадении, работающий обход и
невмешательство в посторонние команды.
