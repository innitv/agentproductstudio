...
10. После получения результата специалиста выполнить **Specialist Output Review**: проверить structured envelope, обязательный artifact, `inputs_used`, `skills_used`, schema readiness, language policy, source/claim status, Surface Output Contract coverage, evidence-to-output mapping, verification evidence и downstream handoff. Markdown без полного artifact output нормализовать или вернуть как `partial`.
...
- **Сверка с образцом измеряется и принадлежит оркестратору.** Есть URL, скриншот или дамп образца — расхождение снимается `yarn reference:metrics` в трёх точках (до выбора основы, сразу после сборки первого экрана, по жалобе «не похоже»), а не оценивается словами: словесный разбор скриншота на run `contractor-payment-demo` был опровергнут измерением по 11 пунктам из 11 проверенных. Процедура, площадка замера (около половины образцов за логином и снимаются зондом в браузере человека) и три класса расхождений — skill `visual-diff-verifier` §3.3.1. Субагенту делегируется прогон инструмента и подготовка таблицы, но НЕ присвоение класса `intentional`: он ставится только со ссылкой на записанное решение, а истории сессии, тел заметок памяти и утверждений человека субагент по устройству не видит. Отдельный агент сверки заводился 2026-08-24 и был снят в тот же день именно из-за этого несоответствия — роль требовала контекста, которого у роли нет. Вердикт `pass`/`fail` по reference fidelity остаётся за `qa-review` на `09-visual-reference`.
...
- **Делегирование только синхронное (`run_in_background: false`).** Процесс живёт один ход, поэтому фоновый субагент погибает вместе с твоим финальным ответом человеку; «фоновый» здесь означает «фоновый внутри хода». Задача дробится под один ход, packet обязан требовать инкрементальной записи на диск, а состояние между ходами читается с диска, а не из предположения, что агент «ещё работает». Замеры и способ перепроверки — `docs/architecture/delegation-lessons.md` §5.
