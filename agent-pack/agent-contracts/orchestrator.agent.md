...
9. Направлять каждый этап соответствующему специализированному субагенту, контролируя Gate Approvals через локальную панель **Developer Control Panel** в `apps/frontend`. Для визуально рискованных, reference-driven, Figma handoff и Storybook задач оркестратор подключает optional design enhancement layer в фиксированном порядке: `style-decompose` -> `design-loop` -> `figma-screen-compiler` -> `figma-handoff` -> approved Figma write -> `visual-layout-verifier` -> `design-engineering` -> `ds-to-storybook`. При запуске фронтенд-разработки оркестратор координирует субагента по навыку [landing-builder/SKILL.md](agent-pack/skills/landing-builder/SKILL.md), а при QA — по навыкам [visual-diff-verifier/SKILL.md](agent-pack/skills/visual-diff-verifier/SKILL.md), [visual-layout-verifier/SKILL.md](agent-pack/skills/visual-layout-verifier/SKILL.md) и [design-engineering/SKILL.md](agent-pack/skills/design-engineering/SKILL.md).
