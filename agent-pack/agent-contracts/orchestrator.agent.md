...
**Строка ставится командой, а не руками:** `yarn workflow:human-review <run-dir> <7.5|8.5a|8.5b> --notes "что сказал человек" [--shown "ссылка/узел/роут"]`. Команда отклоняет пустые замечания и при повторном показе добавляет строку, не заменяя прежнюю. Заведена 2026-08-17: аудит нашёл, что строки не было ни в одном из десяти активных прогонов — проверка работала, а ручная запись не удержалась три прогона подряд.
...
- **Параллелить стадии можно по записи, но не по чтению.** Две стадии, пишущие в разные файлы, запускаются параллельно безопасно; но если вторая **читает** артефакт, который первая в этот момент правит, у неё на входе снимок неизвестной свежести, и её вывод об этом файле выглядит фактом. Либо последовательно, либо в Delegation Packet второму агенту явно назван переписываемый файл и сказано, что выводы по нему перепроверяет оркестратор. Прецедент и разбор — `docs/architecture/delegation-lessons.md` §4.

- 🔴 **Выбор человека — это выбор, а не его вкус. Обобщать выбор в предпочтение и класть обобщение в packet запрещено.** Человек, выбравший из двух вариантов один, сообщил ровно одно: этот вариант лучше того. Он не сообщил правила «ему нравится такое» — а именно правило уезжает субагенту и начинает управлять решениями, которых человек не принимал. Прецедент 2026-08-11 (`a3-brand-presentation-template`): владелец выбрал тёмную версию листа из двух, оркестратор записал в packet «владелец тяготеет к тёмным листам» и задал этим фон следующего листа; владелец поправил прямо — «это не значит, что мне нравится всё тёмное, я сделал выбор из двух». Норма: в packet уходит **сам выбор с его границами** («выбран вариант B для листа X»), а решение однотипного вопроса на другом объекте принимается **от объекта** — что лучше читается, что сильнее работает — и обоснование этого решения возвращается в отчёте. Если предпочтение действительно нужно как правило, оно **спрашивается явно**, а не выводится.
