...
9. Направлять каждый этап соответствующему специализированному субагенту, контролируя Gate Approvals через локальную панель **Developer Control Panel** в `apps/frontend`. Для визуально рискованных и reference-driven задач по умолчанию порядок такой: `style-decompose` -> `design-loop` -> `ds-to-storybook` (витрина компонентов и состояний) -> `design-engineering` -> машинная приёмка (`yarn test-storybook`, `yarn vr:test`, `yarn qa:mobile`). Figma-цепочка `figma-screen-compiler` -> `figma-handoff` -> approved Figma write -> `visual-layout-verifier` подключается дополнительно и только для задач, где Figma-ветка запрошена явно или следует из обоснованного `product_specific|bespoke`. При запуске фронтенд-разработки оркестратор координирует субагента по навыку [landing-builder/SKILL.md](agent-pack/skills/landing-builder/SKILL.md), а при QA — по навыкам [visual-diff-verifier/SKILL.md](agent-pack/skills/visual-diff-verifier/SKILL.md), [visual-layout-verifier/SKILL.md](agent-pack/skills/visual-layout-verifier/SKILL.md) и [design-engineering/SKILL.md](agent-pack/skills/design-engineering/SKILL.md).
...
- **Figma compile/verify loop (только для задач с Figma-веткой):** Figma-ветка включается по явному запросу пользователя или по обоснованному `product_specific|bespoke`, а не по умолчанию; её роль сузилась до дивергентной фазы на `04-design` и разового извлечения решений в токены `design/tokens/`. Если ветка включена: оркестратор не разрешает Figma write без `figma-layout-ir.json`, а после write не передаёт frontend/QA статус `ready`, если отсутствует `figma-visual-qa.json` или `gate_result.ready_allowed=false`. Если ветка не включена, эти гейты фиксируются в ledger как `not_applicable` с причиной, а их место занимает машинная приёмка.
- **Изоляция представлений (Modular Views Architecture):** Оркестратор обязан координировать субагента фронтенда так, чтобы заказная верстка жила в отдельном presentation view внутри `apps/frontend/src/views/` или в отдельном app boundary внутри `apps/<surface>/`, если поверхность имеет собственный домен/build. Для обновления существующего лендинга используй `LandingView.tsx`; для нового самостоятельного продукта допустим отдельный `<ProductName>View.tsx` с тонким подключением в `App.tsx`. Файл [ConsoleView.tsx](apps/frontend/src/views/ConsoleView.tsx) является внутренней панелью управления и не должен модифицироваться кодом лендинга. `App.tsx` должен оставаться легким роутером без содержательной логики представлений.
