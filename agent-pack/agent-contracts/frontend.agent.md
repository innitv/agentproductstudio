...
- **Правила кастомизации темы.** Менять безопасно: цветовые токены, гарнитуру, кольцо фокуса — тема живёт в `design/tokens/shadcn/` и переключается CSS-переменными (`yarn tokens:build`, baseline-гейт `yarn tokens:check`). **Не трогать `--spacing` и шкалу радиусов**: в Tailwind 4 от `--spacing` считаются все отступы и высоты, его сжатие даёт дробные пиксели и ломает выверенный ритм — измерено экспериментом с разделением факторов, метод и числа в `design/tokens/shadcn/README.md`.
- **Тема — свойство проекта.** В верстаке `apps/frontend` объявлена ровно одна тема — `default`, штатный реестр; она точка отсчёта, а не стиль студии. Своя тема заводится под конкретный продукт (файл в `design/tokens/shadcn/` + строка в `THEMES` сборщика + имя в типе `ShadcnTheme`) и уезжает вместе с ним. Правка `default.json` под вкус продукта — дефект: она ломает гейт паритета и лишает отсчёта все остальные проекты.
...
3c1. **Primary App Flow Implementation Gate**: для app/prototype/frontend surface прочитать Primary App Flow Gate из `screens.md` и реализовать P0 walkthrough: entry point, primary action, next state, success evidence и error/recovery path. Без browser/manual/Playwright evidence по этому walkthrough статус не выше `partial`.
...
### Отступление от образца фиксируется, а не решается молча

В reference-driven run образец — источник истины, а его правка — продуктовое решение владельца, не стадии. Любое расхождение с образцом (переименование, замена состояния, отказ от элемента) записывается в отчёт стадии отдельной строкой:

```
reference_deviation: в образце «Документы» → сделано «Раскрытие информации», причина: два термина для одного места
```

и эскалируется оркестратору. Решение может быть по существу верным — фиксация нужна не для запрета, а чтобы владелец образца о нём узнал.

Прецедент (run `a3-shadcn`, 2026-07-29): стадия переименовала пункт меню и заменила шесть активных ссылок на документы выдуманным состоянием «Файл готовится», которого в образце нет. Оба решения были разумны, оба прошли сверку с образцом и финальный QA, и оба нашёл пользователь глазами за минуту. Молчаливое «мы решили лучше» — `process_deviation`.

### Порядок сборки: сначала витрина, потом страница

Внутри стадии соблюдается порядок, от которого зависит гейт человека 8.5a:

1. **компоненты и все состояния** из State Inventory + их истории Storybook — витрина готова;
2. **сообщить оркестратору, что витрина собрана** и страницы ещё нет: здесь человек смотрит «кирпичи» до того, как из них построен «дом»;
3. **композиция страницы** из проверенных компонентов;
4. сообщить, что страница собрана — второй показ.

Почему порядок обязателен, а не «удобнее»: на run `a3-shadcn` (2026-07-29) пять находок пользователя из восьми были дефектами уровня компонента — серые вторичные кнопки, серые поля, тени на кнопках, тени на полях, геометрия обводок. Все они существовали в витрине до сборки страницы в 5714 px, но показ случился один раз и в конце, поэтому композиция строилась из непроверенных кирпичей, а правки вносились в готовую страницу.

Отчёт стадии называет обе точки: когда витрина была готова и когда страница.
