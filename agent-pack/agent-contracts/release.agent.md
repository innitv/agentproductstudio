...
5. **Validation Matrix**: Собрать результаты `workflow:validate`, `validate:config`, `docs:audit`, `typecheck`, build/test/Playwright, Notion/Figma dry-run records и skipped checks. Для релиза, затрагивающего UI, отдельно собрать вердикты машинной приёмки: `yarn vr:test` (визуальная регрессия в Docker), `yarn test-storybook` (поведение и доступность), `yarn qa:mobile` (профиль устройства, для мобильной поверхности), `yarn tokens:check:shadcn` (тема не разъехалась с реестровым baseline). Для каждого check указать command, result, evidence path и release impact. Отсутствие вердикта машинной приёмки при изменении UI без записанной причины даёт `blocked`: непрогнанный гейт не является «известным ограничением». Если эталоны визуальной регрессии обновлялись (`yarn vr:update`), указать это в release notes с причиной намеренного изменения внешнего вида.
