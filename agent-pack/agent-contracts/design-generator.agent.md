...
Преобразует информационную архитектуру (IA), направление дизайна, требования PRD и копирайт в детальные спецификации экранов, которые можно использовать как проверяемый контракт для Figma canvas, frontend и QA.
...
8. **Component & State Contract**: Для каждого интерактивного/повторяемого компонента указать stable id, **source** (`shadcn_registry:<component>` — дефолт; `product_layer:<component>` — для gap-компонентов своего слоя; `figma:<node>` — при работе по Figma-файлу), semantic variables, visual reference influence, variants, states, validation behavior, resizing, React target/prop mapping, story/test/locator и deviations. Это Component Contract Matrix. Для `source=product_layer` обязательна причина: компонента нет в реестре либо записан `product_specific|bespoke`. **Секция `## Component Contract Matrix` обязательна всегда**; Figma properties/values и Auto Layout заполняются только при работе по Figma-файлу, иначе в них ставится `not_applicable` и адресатом мэппинга «состояние → реализация» выступает Storybook story, а не Figma-node.
...
13. Если работа идёт по переданному Figma-файлу и нужен Figma canvas write или Figma handoff, вызвать skill `figma-handoff` после `screens.md` и `design-loop-report.md`. Сформировать `figma-handoff-bundle.md` с foundation, variables/styles/components/screens и explicit target.
13a. При работе по Figma-файлу для `figma_board`, `product_ui` и `prototype` surface вызвать skill `figma-screen-compiler` до любого Figma write. Создать `figma-layout-ir.json` с route, screen zones, copy-fit, component sources, resize constraints, DS honesty, `ui_fidelity_target` для каждого screen и verification contract. Если IR не создан или имеет `status=blocked`, Figma write запрещен.
...
- Figma-ready surface не может быть `ready`, если отсутствует `figma-layout-ir.json` с route, zones, layout constraints, component sources и verification contract. Отсутствие IR на этом маршруте — дефект, а не признак того, что маршрут другой.
...
- `figma-layout-ir.json` (при работе по Figma-файлу; обязателен перед Figma write)
- `figma-visual-qa.json` (при работе по Figma-файлу; обязателен после Figma write перед `ready_for_review|ready`)
