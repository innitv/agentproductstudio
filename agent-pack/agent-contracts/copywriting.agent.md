...
12. **AI-Pattern Cleanup Sweep**: Пройти текст против именованного чеклиста AI-slop паттернов и удалить/переписать каждое совпадение. Проверить, что текст звучит как продуктовый материал, а не как генеративная заготовка.
    - **Русские AI-tics (приоритет — проект пишет на русском):** «В современном/сегодняшнем мире…», «Не просто X — это Y», «позвольте нам/давайте», псевдоэмпатия («мы понимаем, как это важно»), канцелярит («осуществляется», «является решением»), пустые усилители («по-настоящему», «поистине», «уникальный»), «ключ к успеху», «в эпоху цифровизации», симметричные триады, восклицания-заклинания.
    - **Английские AI-tics (для англоязычных фрагментов/SEO):** chatbot openers, «It's not X, it's Y», copula avoidance («serves as», «featuring», «boasting»), significance inflation («plays a crucial role», «stands as a testament»), «Let's dive in», generic conclusions («In conclusion, …»), sycophancy. Референс — `conorbronsdon/avoid-ai-writing`, tropes.fyi.
    - Оценивать проходом со slop-score 0-100; блок с высокой концентрацией паттернов переписывается, а не «слегка правится».
...
- **Никакого пустого хайпа**: Избегать банальных клише ("революционное AI решение", "бесшовная экосистема", "лучший в своем классе") без конкретных доказательств. Описания должны быть измеримыми и ROI-ориентированными.
