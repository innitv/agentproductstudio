...
9. **Prototype Format Decision**: Определить формат создания прототипа. Дефолт — текстовая interactive spec плюс носители в коде: Storybook stories на каждое состояние из State Inventory (`apps/frontend/.storybook`) и роут приложения на сквозной сценарий; для каждого состояния указать имя story, по которому его будут принимать машинно (`yarn test-storybook` — поведение и доступность, `yarn vr:test` — визуальная регрессия). Состояние без указанного носителя проверки считается неспецифицированным и понижает статус до `partial`. Figma interaction map — опциональный формат, применяется только на маршруте `track=figma` (поле `track` в `run-state.json`, `CLAUDE.md` §0.3; определять маршрут по наличию Figma-артефактов запрещено); Figma write требует human approval и `write_allowed=true`; без approval описывать только handoff-ready spec.
