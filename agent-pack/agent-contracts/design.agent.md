...
- **Источник правды для токенов — репозиторий, не Figma.** DTCG-исходники тем shadcn — `design/tokens/shadcn/`, сборка `yarn tokens:build`, baseline-гейт `yarn tokens:check`.
...
- **Роль Figma сузилась до двух сценариев:** дивергентная фаза на `04-design` (подобрать вид, показать человеку) и разовое извлечение решений в токены `design/tokens/`. Постоянной синхронизации Figma↔код нет.
- **Маршрут производства макета — ось `track`, и её выбирает человек на `00-intake`** (`CLAUDE.md` §0.3): ответ на вопрос «Нужен макет в Figma перед вёрсткой?» пишется в `run-state.json`, и Figma-часть контракта ниже действует только при `track=figma`. Это **заменяет прежнее правило** «Figma-ветка включается по явному запросу или по обоснованному `product_specific|bespoke`»: `design_system_mode` и `track` ортогональны — своя дизайн-система штатно проектируется в коде (Storybook как витрина), и выбор `product_specific|bespoke` на `04-design` **не включает** Figma-маршрут и не переопределяет ответ человека, данный на нулевой стадии. Если продукту действительно нужен холст Figma, это открытый вопрос к пользователю и смена оси на intake, а не решение агента; смену задним числом после отработки `06-screens`/`08-frontend` валидатор отклонит.
...
- `run-state.json` — оси запуска; маршрут макета берётся из поля `track` (`code`/`figma`)
...
1b. **Token Precedence Rule**: при выборе значений токенов соблюдать явный порядок источников: (1) явная спецификация из PRD/reference-analysis/STYLE_GUIDE → (2) токены репозитория (`design/tokens/shadcn/`, собираются `yarn tokens:build`) → (3) bound variables Figma-DS (`design/figma/<slug>/foundation.md`), только на маршруте `track=figma` → (4) документация/дефолты. Источник правды для токенов — репозиторий, а не Figma: значение из Figma переносится в `design/tokens/` разовым извлечением и дальше живёт там. Если источники токенов противоречат друг другу (например reference-цвет ≠ токен темы) — **не выбирать молча**, а зафиксировать конфликт и эскалировать пользователю/оркестратору как открытый вопрос.
...
8a. Для `extend|product_specific` **на маршруте `track=figma`** зафиксировать Two-Pass Figma Build: сначала `visual_calibration` на 2-3 экранах, затем `systemization`; component matrix нельзя масштабировать до visual verdict. На маршруте `track=code` (умолчание студии) тот же двухпроходный порядок применяется в коде: сначала 2-3 экрана как composition stories в Storybook и визуальный вердикт человека, затем систематизация токенов/компонентов; в `design-brief.md` записать, какой из двух путей выбран и почему.
...
3. `figma-handoff` — только на маршруте `track=figma`: после `screens.md` и `design-loop-report.md` при наличии, перед любым Figma write. Нужен для foundation/components/screens bundle и approval gate. На маршруте `track=code` навык вне маршрута и не вызывается; запись — `skipped_with_reason=track_code` в `handoff-bundle.md` (прежнее значение `figma_branch_not_used` переименовано вместе с осью).
...
5. `ds-to-storybook` — после frontend. На маршруте `track=code` это основная витрина компонентов и состояний, а не опция: именно stories принимаются машинно (`yarn test-storybook` — поведение и доступность, `yarn vr:test` — визуальная регрессия в Docker с машинным вердиктом).
