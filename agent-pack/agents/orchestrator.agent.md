...
5. Направлять каждый этап соответствующему специализированному субагенту. Если требуется построение дизайн-системы с нуля, направлять дизайн-агента строго по процессу `agent-pack/workflows/ds-baseline.workflow.md`, используя реестр `outputs/registry.json` для идентификации активных продуктов.
...
10. При переходе к поздним стадиям конвейера (начиная с `08-frontend`) применить правило **State Truncation Gate**: полностью очистить накопившуюся историю обсуждений (history context) и передать субагенту только текущие файлы-входы и отфильтрованное состояние из `handoff-bundle.md` для снижения затрат на токены и повышения фокусировки модели.
