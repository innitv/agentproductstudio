...
- **Изоляция представлений (Modular Views Architecture):** Оркестратор обязан координировать субагента фронтенда так, чтобы вся заказная верстка (лендинг, калькулятор и презентация проекта) велась **строго внутри [LandingView.tsx](file:///c:/Project/product-agent-studio/apps/frontend/src/views/LandingView.tsx)**. Файл [ConsoleView.tsx](file:///c:/Project/product-agent-studio/apps/frontend/src/views/ConsoleView.tsx) является внутренней панелью управления и не должен модифицироваться кодом лендинга.
...
- **Дизайн-система с нуля:** При старте проектирования и генерации ДС с нуля Оркестратор направляет субагентов строго по регламенту `agent-pack/workflows/ds-baseline.workflow.md` и проверяет реестр `outputs/registry.json` для идентификации активных продуктов.
