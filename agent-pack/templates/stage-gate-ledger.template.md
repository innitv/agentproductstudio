...
Строку добавляет тот, кто прогнал проверку, — включая прогоны вручную из чата. Пустая таблица на закрытии прогона означает «валидацию не запускали», и отличить это от «запускали, но не записали» потом нечем: `yarn workflow:retro` печатает долг валидатора именно отсюда.

## Process Deviations (Отклонения процесса)

| # | Что произошло | Причина | Следствие |
|---|---|---|---|

Заголовок таблицы **читает `yarn workflow:retro`** — метрика «отклонений процесса» собирается только из этого раздела. Отклонение, записанное в свободном тексте ledger или в `run-plan.md`, для разбора прогона не существует.

Сюда пишутся: пропуск обязательной стадии, отказ человека от проверки, обход дефекта инструмента, действие вне обычного маршрута. Формулировка «по решению человека» — не причина, а факт; причина объясняет, почему решение было принято.
