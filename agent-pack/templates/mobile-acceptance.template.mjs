...
const CONFIG = {
  ...
  baseUrl: "<ЗАПОЛНИ: http://127.0.0.1:4173 — адрес превью или деплоя>",
  ...
  app: {
    ...
    scrollContainer: "<ЗАПОЛНИ: селектор скроллящегося контейнера или \":root\">",
  ...
  scenarios: {
    ...
    touch_scroll_from_content: {
      route: "<ЗАПОЛНИ: маршрут главного экрана, например \"/\">",
      prelude: [],
      /** Свайп ОБЯЗАН начинаться на интерактивном элементе, не на пустом фоне. */
      touchTarget: "<ЗАПОЛНИ: селектор карточки/строки/кнопки внутри списка>",
      /**
       ...
       */
      horizontalScroller: "<ЗАПОЛНИ: селектор горизонтального ряда или null>",
    ...
    safe_area_and_fixed: {
      route: "<ЗАПОЛНИ: маршрут экрана с закреплённой панелью>",
      prelude: [],
      /** Закреплённые панели: не должны заезжать за границы вьюпорта. */
      stickyPanels: ["<ЗАПОЛНИ: селектор sticky/fixed панели>"],
      /** Ожидаемый `<meta name=\"theme-color\">`; null — тег не используется. */
      themeColor: "<ЗАПОЛНИ: \"#ffffff\" или null>",
      /** Требовать ли `viewport-fit=cover` в meta viewport. */
      requireViewportFitCover: true,
      /** Ожидаемый цвет канвы в системных зонах, [r, g, b]. */
      expectedCanvasColor: "<ЗАПОЛНИ: [255, 255, 255]>",
    ...
    overlay_appearance: {
      route: "<ЗАПОЛНИ: маршрут, с которого вызывается оверлей>",
      /** Шаги до состояния, из которого оверлей вызывается. */
      prelude: [],
      /** Что нажать, чтобы оверлей появился; null — если он появляется сам. */
      trigger: "<ЗАПОЛНИ: селектор кнопки-триггера или null>",
      /** Сам оверлей: обязан появиться и быть виден целиком. */
      overlay: "<ЗАПОЛНИ: селектор оверлея/баннера/модалки>",
      /**
       ...
       */
      layoutAnchor: "<ЗАПОЛНИ: селектор элемента, который не должен сдвинуться>",
    ...
    composition_widths: {
      route: "<ЗАПОЛНИ: маршрут визуально плотного экрана>",
      prelude: [],
      ...
      anchors: ["<ЗАПОЛНИ: селекторы ключевых элементов композиции>"],
      /**
       ...
       */
      mustNotOverlap: [["<ЗАПОЛНИ: селектор A>", "<ЗАПОЛНИ: селектор B>"]],
    },
    ...
    scroll_position_on_state_change: {
      route: "<ЗАПОЛНИ: маршрут экрана со сменой состояния>",
      prelude: [],
      /** Куда отмотать перед действием: число px или "third" (треть хода). */
      scrollTo: "third",
      /** Действие, меняющее состояние. */
      trigger: "<ЗАПОЛНИ: селектор кнопки, меняющей состояние>",
      /** Что должно появиться после действия (признак смены состояния). */
      appears: "<ЗАПОЛНИ: селектор появляющегося элемента>",
      settleMs: 1500,
    },
  },
};
