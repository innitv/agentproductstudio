...
- External research provider calls с project/product context, кроме обязательных DeepSeek/Gemini cross-check на `01-research`.
- Деплой.
- Внешние сообщения.
- Изменение секретов.
- Удаление данных.
...
Исключение для research: DeepSeek/Gemini cross-check на `01-research` запускается автоматически без отдельного approval, если провайдер доступен. Разрешенный scope: contradiction review, claims-to-validate, проверка логики и пробелов исследования. Запрещенный scope: публикация, запись во внешние системы, deploy, git write, изменение секретов, отправка сообщений или использование provider synthesis как source-backed evidence. Результат или failure обязательно фиксируется в run ledger.
...
- Если пользователь или workflow явно требует Tavily/DeepSeek/OpenAI Docs MCP/Notion/visual reference screenshot gate, не обходи это другим инструментом. Для DeepSeek/Gemini на `01-research` запускай auto-run cross-check; для остальных approval-gated providers запроси approval или верни blocker.
