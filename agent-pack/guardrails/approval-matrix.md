...
| External research API call with project/product context | `external_research_provider_call` | required with exact provider/run target unless user already approved this provider for the run; not required for mandatory DeepSeek/Gemini cross-check on `01-research` |
...
Exception: mandatory DeepSeek/Gemini calls on `01-research` are auto-run research cross-checks, not approval-gated provider opt-ins. They may only be used for contradiction review and claims-to-validate, must not publish or mutate any external surface, and must be logged in the run ledger. If unavailable, record provider failure and keep the stage partial.
...
- для `model_provider_call` и `external_research_provider_call` вопрос должен явно называть provider, stage/run target и какие данные покинут локальную песочницу; исключение — обязательные DeepSeek/Gemini cross-check на `01-research`, которые запускаются автоматически и логируются без approval-вопроса;
