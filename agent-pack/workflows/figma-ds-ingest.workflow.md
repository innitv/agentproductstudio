...
- **Кит shadcn/ui в Figma уже заингестен — повторно не ингестить.** С 2026-07-28 в реестре есть `shadcn-ui-community` (file key `pCDj1p7ItjKJcXPJZDqXi6`): 172 компонента и сета с Node ID и ключами, семантические переменные, 14 125 иконок. Если задача — макеты на shadcn в Figma, читай `design/figma/shadcn-ui-community/components.md`, а не запускай ingest заново.
  - 🔴 **В `archive/design-systems/shadcn-ui-components-2026/` лежит индекс ЭТОЙ ЖЕ библиотеки, но другого файла** (community-оригинал `NUoNEuTJ3OZOGH2c780Z55`, июнь 2026). **ID страниц там совпадают с действующим китом, а Node ID компонентов — нет** (`Badge` `73:3479` против `665:2024`, `Switch` `76:10548` против `76:10618`). Совпадение имён страниц создаёт ложное ощущение взаимозаменяемости: архивный Node ID либо не найдётся, либо укажет на другую ноду. Адресация берётся только из действующего индекса; из архива можно смотреть deep profiles в `components/` как справку по анатомии, перепроверяя каждый ID.
