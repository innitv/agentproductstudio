...
- Design/Figma/frontend handoff по умолчанию идёт **без Figma**: витрина продуктового UI — Storybook в коде, токены — DTCG в `design/tokens/`. Отсутствие Figma-артефактов в задаче, которая через Figma не идёт, — штатный маршрут, а не `skipped_with_reason`.
...
| `06-screens` | `design-generator` | `prd.md`, `ia-brief.md`, `design-brief.md`, `copy-deck.md` | `screens.md` с Component Contract Matrix и Frame/State Implementation Map; экран описывается как composition story (роут приложения), а не как отдельный «макет для витрины»; optional `design-loop-report.md`; только для Figma-маршрута — `figma-layout-ir.json`, `figma-handoff-bundle.md`, `figma-visual-qa.json` после approved Figma write | Prototype, frontend, visual reference review, QA. |
...
## Передача витрины и приёмки (дефолтный маршрут)
...
## Передача Figma ↔ frontend (только для Figma-маршрута)

Раздел применяется, когда задача действительно идёт через Figma: пользователь дал Figma-файл, требуется canvas write или разовое извлечение токенов. Для дефолтного маршрута он не применяется, и отсутствие Figma-артефактов пропуском не считается.
