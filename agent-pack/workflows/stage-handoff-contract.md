...
- Design/Figma/frontend handoff по умолчанию идёт **без Figma** (`track: code` в `run-state.json`): витрина продуктового UI — Storybook в коде, токены — DTCG в `design/tokens/`. Маршрут-условные Figma-**артефакты** на этом маршруте не создаются и записи в ledger не требуют вовсе; маршрут-условные **секции** `screens.md`/`frontend-result.md` закрываются строкой `skipped_by_track` в таблице «Секции вне маршрута» `stage-gate-ledger.md`. `skipped_with_reason: Figma не участвует` писать запрещено. Каноническая формулировка — `agent-pack/workflows/claude-operating-rules.md` §5, раздел «Маршрут (`track`)».
...
- Для `track: figma` обязательны visual calibration до systemization, `figma-layout-ir.json` до Figma write, `figma-visual-qa.json` после write, Component Contract Matrix и frame/state -> route/story/component mapping.
...
| `06-screens` | `design-generator` | `prd.md`, `ia-brief.md`, `design-brief.md`, `copy-deck.md` | `screens.md` с Component Contract Matrix и Frame/State Implementation Map; экран описывается как composition story (роут приложения), а не как отдельный «макет для витрины»; optional `design-loop-report.md`; только для `track: figma` — `figma-layout-ir.json`, `figma-handoff-bundle.md`, `figma-visual-qa.json` после approved Figma write | Prototype, frontend, visual reference review, QA. |
...
## Передача витрины и приёмки (маршрут `track: code`, дефолт)

Дефолтный маршрут продуктового UI не проходит через Figma (CLAUDE.md §6.1, ось `track` — §0.3). Передача выглядит так:
...
## Передача Figma ↔ frontend (только для `track: figma`)

Раздел применяется на маршруте `track: figma`: пользователь дал Figma-файл, требуется canvas write или разовое извлечение токенов. Для `track: code` он не применяется, отсутствие Figma-артефактов пропуском не считается и записи не требует.
