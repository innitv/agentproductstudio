...
- **Дизайн-система по умолчанию — shadcn/ui в коде** (CLAUDE.md §6.1), витрина — Storybook, приёмка — машинная (`yarn vr:test`, `yarn test-storybook`, `yarn qa:mobile`). Наблюдения ниже, сформулированные как «Figma/design-system agent tooling», применяются только при работе по переданному Figma-файлу.
