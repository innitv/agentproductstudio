...
До генерации, записи, публикации, handoff downstream-агенту или финального статуса действует **Thoroughness-First Gate**: stage оптимизируется на внимательность, полноту context/source inventory и соблюдение правил, а не на скорость. `quick draft` допустим только по явной фразе пользователя. Исполнитель обязан проверить существующие инструкции, артефакты, templates, design-system assets, components, runtime contracts и references; новое создается только для доказанного gap. Нарушение уже существующего правила записывается как `process_deviation`, а не как "поправка пользователя".
...
Проект работает через Claude Code внутри IDE/чата: Claude читает инструкции агентов, workflow-документы и шаблоны, затем выполняет этапы через локальные инструменты и фиксирует артефакты.
