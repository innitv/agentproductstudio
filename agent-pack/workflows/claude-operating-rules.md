...
Практическое следствие для гейтов ниже: **Figma-гейты не отменены и остаются обязательными для задач, где Figma реально используется** (пользователь дал Figma-файл, требуется canvas write, идёт извлечение токенов). Но отсутствие Figma-артефактов в задаче, которая через Figma не идёт, — это не пропуск и не `skipped_with_reason`, а штатный маршрут. Не создавай Figma-стадию только ради заполнения чек-листа.
...
Этот gate заменяет ручную сверку макета с Figma, а не дополняет её: если Figma в задаче не участвует, парная сверка с Figma не требуется, а машинная приёмка — требуется всегда.

### Two-Pass Figma Build Gate

Применяется, когда задача **действительно идёт через Figma** (дивергентный черновик на `04-design`, разовое извлечение токенов, показ человеку, работа с переданной пользователем Figma-библиотекой). Для дефолтного маршрута `reuse` shadcn/ui Figma-сборка не требуется, и её отсутствие не фиксируется как пропуск.
...
- **Дефолтный маршрут (`reuse` shadcn/ui, Figma не участвует):** источник — элемент официального реестра или собственный компонент из своего слоя; доказательством контракта служат история Storybook с покрытием состояний и её снапшот визуальной регрессии, а не Figma-нода. Отдельный `figma-handoff-bundle.md` в этом маршруте не создаётся, и его отсутствие не является пропуском.
- **Figma-маршрут:** источник — `Figma component/property` (file/node/component key), и матрица дополнительно живёт в `figma-handoff-bundle.md`.
