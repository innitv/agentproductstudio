...
**Это проверяется машинно.** `yarn workflow:validate` после отработавшей `08-frontend`
требует обе строки: без них `08`, `09` и `11` не закрываются как `success`
(`validateHumanReviewGates` в `runtime/typescript/validate-workflow-run.ts`, регрессия —
`workflow:test-validator`). Машина не проверяет, что человек посмотрел; она проверяет,
что ему показывали — именно этого шага и не было на `a3-shadcn`.
