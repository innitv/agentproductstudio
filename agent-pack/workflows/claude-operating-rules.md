...
<a id="track-gate"></a>
### Маршрут (`track`) и как оформляется «Figma не участвует»
...
- **Маршрут — ось запуска, а не вывод по файлам.** `track` живёт в `run-state.json` (`code` — дефолт студии: спецификация экранов + shadcn/ui + Storybook; `figma` — макет на холсте Figma), фиксируется на `00-intake` ответом человека и дублируется в поле `Track` шапки `stage-gate-ledger.md`. Полное описание оси — CLAUDE.md §0.3. **Определять маршрут по наличию `figma-layout-ir.json` или `figma-handoff-bundle.md` запрещено:** Figma-run, не создавший файл, выглядел бы как честный код-маршрут и обошёл бы гейт. Маршрут не меняется задним числом — валидатор отклонит run, где `06-screens`/`08-frontend` уже отработали под другим маршрутом.
...
- **Маршрут-условные секции закрываются `skipped_by_track`.** Секции `## Layout Compiler Contract`, `## Figma Readiness` в `screens.md` и `## Design System Implementation`, `## Component Contract Implementation`, `## Frame / State Implementation Map`, `## Figma Visual QA Gate Summary`, `## Figma Roundtrip Deviations` в `frontend-result.md` перечисляются в `stage-gate-ledger.md` в таблице «Секции вне маршрута» строкой со статусом `skipped_by_track`, называющей стадию и секцию. Запись проверяется машинно в три стороны: пропуск секции, которую маршрут требует, — ошибка; пропуск секции, которой нет ни в одном маршруте, — ошибка (протухшая запись); снятая маршрутом секция **без** строки в ledger — тоже ошибка, как только стадия отдала артефакт. Точный список даёт `yarn workflow:validate <run-dir> --track <track>`.
...
Этот gate заменяет ручную сверку макета с Figma, а не дополняет её: на маршруте `track: code` парная сверка с Figma не требуется и записи не требует, а машинная приёмка обязательна на **обоих** маршрутах.

### Two-Pass Figma Build Gate

Применяется на маршруте `track: figma` — когда задача **действительно идёт через Figma** (дивергентный черновик на `04-design`, разовое извлечение токенов, показ человеку, работа с переданной пользователем Figma-библиотекой). На маршруте `track: code` Figma-сборка не требуется, и её отсутствие не фиксируется как пропуск (см. [Маршрут (`track`)](#track-gate)).
...
- Для `reuse|extend` **на Figma-маршруте** (`track: figma`, источником DS является Figma-библиотека) зафиксируй `selected_design_system_slug` из `design/figma/registry.json` и работай с выбранной системой: сначала локальный индекс `design/figma/<design-system-slug>/`, затем `yarn figma:audit --registry design/figma/<design-system-slug>/component-contracts.json` после изменения component sets или React API. Live-аудит со статусом `needs_revision|blocked` запрещает закрывать Figma roundtrip как `success` без deviation/waiver. **На дефолтном маршруте (`track: code`, `reuse` shadcn/ui в коде) реестра нет и он не требуется:** выбранная система — `apps/frontend/src/components/shadcn/`, её аудит — `yarn tokens:check`, `yarn test-storybook` и `yarn vr:test`. Требовать `selected_design_system_slug` или `figma:audit` вне Figma-маршрута запрещено — там нечего аудировать.
