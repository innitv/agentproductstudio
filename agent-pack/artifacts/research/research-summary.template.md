...
## Scenario User Flows Handoff

Required: отдельный файл `scenario-user-flows.md` должен раскрывать P0/P1 сценарии как реальные флоу. В этом summary оставь только индекс и ссылки, полная проработка живет в отдельной странице.

| Флоу | Где раскрыт | Что обязательно покрыто | Статус |
|---|---|---|---|
| F01 | `scenario-user-flows.md` | персона, ситуация, шаги, статус, исключение, доказательство, проверка | open / covered / skipped_with_reason |
