...
3. Зафиксируй в `run-plan.md`: тип работы, профиль (`standard`/`reference`), маршрут стадий и non-goals.
...
7. **Незакрытое — записывается.** Пропущенный слой → `skipped_with_reason`. Недоступный provider/approval → `blocked`/`partial`. Молчаливый пропуск запрещён.
...
- [ ] Все пропуски записаны как `skipped_with_reason`, `partial` или `blocked`.
