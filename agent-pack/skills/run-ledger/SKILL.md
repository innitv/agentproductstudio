...
3b. Секции, которые исключает **маршрут**, перечисли сразу в `stage-gate-ledger.md` в таблице «Секции вне маршрута» со статусом `skipped_by_track`; строка обязана называть стадию и секцию. Маршрут пишется в `run-state.json` (`track`) и определять его по наличию `figma-layout-ir.json` запрещено: Figma-run, не создавший файл, выглядел бы как честный код-маршрут. Запись проверяется машинно в три стороны — ошибку валидатора дают: пропуск секции, которую маршрут требует; пропуск секции, которой нет ни в одном маршруте (протухшая запись); и снятая маршрутом секция БЕЗ строки в ledger, как только стадия отдала свой артефакт (незакрытое ожидание неотличимо от забытой секции). Точный список даёт `yarn workflow:validate <run-dir> --track <track>`.
