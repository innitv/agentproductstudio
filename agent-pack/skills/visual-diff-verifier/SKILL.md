...
**Узкий desktop-вьюпорт не заменяет устройство.** Эмуляция ширины не воспроизводит touch-события, safe-area и поведение `visualViewport`. В том run четыре бага (заблокированная прокрутка при касании, обрезанный баннер, съехавшая композиция, цвет системных зон) прошли все desktop-проверки и всплыли только на реальном iPhone. Мобильная приёмка = профиль устройства с `isMobile: true` и `hasTouch: true` плюс настоящие тач-жесты, а не `setViewportSize`. Помни границу: Chromium с профилем iPhone — это не WebKit; специфику Safari (`theme-color`, тонирование системных панелей, захват оси вложенным скроллером) локально не воспроизвести, финальное подтверждение даёт только устройство.
...
- [ ] Для мобильной поверхности приёмка прогнана в профиле устройства (`isMobile` + `hasTouch`), а не в узком desktop-вьюпорте; ограничение «Chromium ≠ WebKit» записано.
