...
1. **Закрывается ли задача темой поверх shadcn?** Цвет, гарнитура и кольцо фокуса меняются свободно через `design/tokens/shadcn/` и `yarn tokens:build:shadcn` — два шрифтовых токена дают больше характера, чем два десятка геометрических. Если да, этот skill не применяется: собственный foundation здесь будет дублированием.
