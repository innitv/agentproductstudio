...
**Skill применяется только на маршруте `track: figma`** — когда `run-state.json` объявляет маршрут `figma`: пользователь просит макет в Figma или показ результата человеку. По `CLAUDE.md` §6.1 Figma сузилась до дивергентной фазы на `04-design` и разового показа; Figma-кит компонентов не ведётся, синхронизации с кодом нет. Маршрут читается из `run-state.json` (CLAUDE.md §0.3), а не выводится по наличию файлов.
...
- **Handoff не является предусловием frontend.** Код собирается из `design/tokens/` и shadcn; bundle описывает Figma-поверхность.

**Как оформляется «Figma не участвует» (на маршруте `track: code`).** `figma-handoff-bundle.md` — маршрут-условный **артефакт**: он не создаётся и **записи в ledger не требует вовсе**, это штатный маршрут, а не пропуск. Писать `skipped_with_reason: Figma не участвует` **запрещено** — `skipped_with_reason` означает пропущенный применимый слой, а не неприменимый маршрут. Записи требуют только маршрут-условные **секции** `screens.md`/`frontend-result.md` — строкой `skipped_by_track` в таблице «Секции вне маршрута» `stage-gate-ledger.md` с указанием стадии и секции. Каноническая формулировка — `agent-pack/workflows/claude-operating-rules.md` §5, раздел «Маршрут (`track`) и как оформляется „Figma не участвует“» (заменяет прежнее указание этого skill про `skipped_with_reason`).
