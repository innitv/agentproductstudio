 ...
 * Это ВТОРАЯ реализация того же экрана. Первая — `CardRequestView.tsx` на
 * дизайн-системе A3. Продуктовая логика, справочники, тексты и правила
 * валидации у них общие: оба берут `card-request.data.ts`, копии данных нет.
 * Отличается только основание интерфейса, и в этом весь смысл пары — она даёт
 * прямое сравнение двух основ на одинаковом содержании.
 *
 * ─── ЧЕМ ОТЛИЧАЕТСЯ ПОДХОД К ВЁРСТКЕ ────────────────────────────────────────
 * У A3-версии 528 строк собственного CSS (`styles/card-request.css`): она
 * раскладывает готовые компонентные классы. Здесь собственного CSS нет вовсе —
 * раскладка живёт в утилитах Tailwind прямо в разметке. Это не «лучше», это
 * другая цена: правка отступа в A3 меняется в одном месте на все экземпляры,
 * здесь — в каждом месте отдельно.
 *
...
export function CardRequestShadcnView({
  ...
  return (
    <ShadcnThemeScope className="min-h-dvh pb-28" theme={theme}>
      {/*
        Отступ снизу задан вручную и это вынужденная мера. В A3-версии тост
        лежит ВНУТРИ дока над панелью действий, поэтому их взаимное положение
        обеспечено композицией. sonner рендерится в портале с `position: fixed`
        и о существовании панели не знает — без смещения уведомление ложится
        поверх кнопок. Число (72px) — высота панели плюс зазор.
      */}
      <Toaster
        offset={{ bottom: 72 }}
