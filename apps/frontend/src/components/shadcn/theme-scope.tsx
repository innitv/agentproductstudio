 ...
 *
 * Тема здесь ровно одна — `default`, штатный реестр shadcn: опорная точка
 * отсчёта, по которой в витрине виден компонент «как из коробки».
 *
 * Продуктовых тем в верстаке не держим. `a3` и `portfolio` жили здесь до
 * 2026-08-03 и уехали вместе со своими экранами: тема — свойство продукта и
 * принадлежит его репозиторию (`shadcn-library` §3.3). Пока продукт собирается
 * в студии, его тема живёт тут; как только он переехал — уезжает с ним, иначе
 * в двух местах остаются две копии, которые разойдутся.
 *
