 ...
 *   node apps/contractor-payment-demo/tests/verify.mjs \
 *     --base=http://127.0.0.1:4319 \
 *     --out=outputs/contractor-payment-demo/2026-07-23/screenshots
 *
...
const OUT = path.resolve(here, "../../..", args.out ?? "outputs/contractor-payment-demo/2026-07-23/screenshots");
...
// ═══ Шаг 1 маршрута и отдельный короткий сценарий S-C ════════════════
{
  ...
  for (const slug of ["flowwow-like", "uchi-like"]) {
    const data = await withPage(392, `${BASE}/?tenant=${slug}`, async (page) => {
      ...
      const selectedAfterTap = await page.locator(ozon).getAttribute("data-selected");

      // Выбор «Ozon Банк» раскрывает блок проверки телефона.
      await page.waitForSelector('[data-testid="phone-block"][data-expanded="true"]', {
        timeout: 3000,
      });
      const expanded = true;

      const widthBefore = (await page.locator(cta).boundingBox()).width;
      ...
      const widthLoading = (await page.locator(cta).boundingBox()).width;

      // Решение №2: после проверки приходит ПУШ поверх экрана подрядчика,
      // служебного оверлея между ними нет.
      ...
      // Свайп вверх возвращает на экран подрядчика с сохранённым выбором.
      ...
      const selectionKept = await page.locator(ozon).getAttribute("data-selected");
      const ctaState = await page.locator(cta).getAttribute("data-state");
      const digitsKept = await page.locator('[data-testid="phone-input"]').inputValue();
    ...
    const passed =
      ...
      data.openingLabel === "Открываем Ozon Банк…" &&
      data.widthBefore === data.widthLoading &&
      data.bannerGone &&
    ...
    if (!passed) ok = false;
    rows.push(
      `${slug}: выбор→${data.selectedAfterTap}, блок раскрыт=${data.expanded}, проверка «${data.checkingLabel}»→открытие «${data.openingLabel}», ширина ${data.widthBefore}→${data.widthLoading}, свайп вверх убрал баннер=${data.bannerGone}, выбор сохранён=${data.selectionKept}, номер сохранён «${data.digitsKept}», CTA=${data.ctaState}, цифр в аналитике нет`,
    );
  }
...
// ═══ Инвариант положения кнопки на O-2 ═══════════════════════════════
{
  ...
  long.display_name = "Кондитерская мастерская «Тёплый хлеб и мёд»";
...
// ═══ Три коррекции контраста экранов банка ═══════════════════════════
{
    ...
    withPage(392, `${BASE}/?tenant=uchi-like&stage=bank_success&a11y=${a11y}`, (page) =>
      page.evaluate(() => {
        ...
        return {
          ...
          gradientPin: style.getPropertyValue("--bank-gradient-pin").trim(),
          gradientPinH: style.getPropertyValue("--bank-gradient-pin-h").trim(),
  ...
  const ok =
    ...
    enforced.gradientPin.toLowerCase() === "#005aff" &&
    enforced.gradientPinH === "120px" &&
    donor.textSecondary.toLowerCase() === "#6f7881" &&
    donor.success.toLowerCase() === "#0bcc59" &&
    donor.gradientPinH === "0px";

  record(
    "Три коррекции контраста экранов банка применяются при enforced",
    ok,
    `enforced: text_secondary ${enforced.textSecondary} (4.19→5.18), success ${enforced.success} (2.14→3.37), верх градиента ${enforced.gradientPin} на ${enforced.gradientPinH} (4.28→5.38), бейдж ${enforced.badgeFill} | ` +
      `donor_faithful: ${donor.textSecondary} / ${donor.success} / пришпиливания нет (${donor.gradientPinH})`,
  );
}
...
// ═══ Полный путь в обеих темах ═══════════════════════════════════════
{
  ...
  for (const slug of ["flowwow-like", "uchi-like"]) {
    ...
    if (!passed) ok = false;
    rows.push(
      `${slug}: push→splash(${data.splashDuration} ms)→оплата ${data.bankAmount} «${data.merchant}»→успех «${data.returnLabel}»→«${data.paidTitle}»→рестарт в ${data.finalStage} без перезагрузки (navigation entries ${data.reloaded})`,
    );
  }
  record("Полный путь проходится в обеих темах, рестарт без перезагрузки", ok, rows.join(" | "));
}
...
// ═══ Граница темы на экранах банка (рантайм, в обе стороны) ══════════
{
  ...
  record(
    "Граница темы двусторонняя: банк без --t-*, возврат без --bank-*",
    ok,
...
// ═══ Проверка клиентства: формат, результат, инвариант, приватность ══
{
  ...
  for (const slug of ["flowwow-like", "uchi-like"]) {
    ...
    const totalSel =
      layout === "A" ? '[data-testid="total-row"]' : '[data-testid="totals-payable"]';
    ...
    const r = await withPage(
      ...
      async (page) => {
        ...
        const yHint = await measureY(totalSel);
        ...
        const yError = await measureY(totalSel);
    ...
    rows.push(
      `${layout}/${slug}: пусто→«${r.emptyMsg}» (${r.emptyStage}), неполный→«${r.incompleteMsg}», демо-номер→«${r.notClientMsg}» (${r.errorStage}); ` +
        `инвариант «Итого» y hint=${r.yHint} error=${r.yError} (совпало ${invariant}); storage ls=${r.storage.ls}/ss=${r.storage.ss}; цифр в аналитике нет`,
    );
  }
  record(
...
const shots = [
  ...
  ["a-flowwow-like-392-cta-loading", 392, "?tenant=flowwow-like&state=cta_loading"],
